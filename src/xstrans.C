...
int xs_trans::_find_intersects(long int yy, long int *working) {
  ...
  for (jj=0; jj<_num_segs; jj++) {
    if ( _segs[jj]._y1 > yy ) break;   /* we are done ! */

    if ( _segs[jj]._y1 == yy && _segs[jj]._y2 == yy ) {   /* this is special case! */
      working[cntr++] = _segs[jj]._x1;
      working[cntr++] = _segs[jj]._x2;   /* counted as two! */
    } else if ( _segs[jj]._y1 < yy && _segs[jj]._y2 > yy )  {
      slope = ((double)_segs[jj]._x2 - (double)_segs[jj]._x1)/((double)_segs[jj]._y2 - (double)_segs[jj]._y1 );
      working[cntr++] = _segs[jj]._x1 + (long int)( ((double)yy - (double)_segs[jj]._y1) *slope );
  ...
  for (jj=0; jj<cntr; jj++) {
    for (kk=jj+1; kk<cntr; kk++) if ( working[jj]!=working[kk]) break;
    if ( kk==jj+1) continue;
    for (ll=jj+1; kk<cntr; kk++, ll++) working[ll]=working[kk];
  }
  ncntr=1;
  for (jj=1; jj<cntr; jj++) ncntr = working[jj]==working[jj-1] ? ncntr : ncntr+1;
...
double xs_trans::_get_pp(int num_now, long int *now, int num_prev, long int *prev, double dy, double unit) {
  int jj,kk, num_tt;
...
void xs_trans::_find_min_max() {
  ...
  mymin = _ryy[0];
  for (jj=1; jj<_num_pts; jj++) mymin = mymin < _ryy[jj] ? mymin : _ryy[jj];
...
int xs_trans::_quick_check() {
  ...
  // x-coordiates have to be monotonically increasing, this might be relaxed in the future
  // as far as the channel doesn't completely close!
  rc = 0;
  for (jj=1; jj<_num_pts; jj++) {
    _tmp[jj] = _yyin[jj] > _yyin[jj-1] ? 1 :  ( _yyin[jj] < _yyin[jj-1] ? -1 : 0 );
    if ( _xxin[jj] < _xxin[jj-1] ) { rc = -1; break; } // equal is OK 
  }
  if ( rc == -1 ) {
  ...
  for (jj=1; jj<_num_pts-1; jj++) {
    _tmp[jj] = (_yyin[jj-1]>=_yyin[jj] && _yyin[jj+1]>=_yyin[jj]) ? 0 : _tmp[jj];
    _tmp[jj] = (_yyin[jj-1]<=_yyin[jj] && _yyin[jj+1]<=_yyin[jj]) ? 2 : _tmp[jj];
  }
...
void xs_trans::PrintDenseAll(FILE *f, const char *head) {
  ...
  for (jj=0; jj<_num_samples; jj++) {
    fprintf(f, "%s %.4e %.4e %.4e %.4e %.4e\n", head, _my_aa[jj], _my_yy[jj], _my_ll[jj],_my_cc[jj],_my_pp[jj]);
  }
}
...
void xs_trans::PrintSparseAll(FILE *f, const char *head) {
  ...
  for (jj=0; jj<_final_samples; jj++) {
    fprintf(f, "%s %.4e %.4e %.4e %.4e %.4e\n", head, _aa[jj], _yy[jj], _ll[jj],_cc[jj],_pp[jj]);
  }
}
