...
public:
  ...
  int IsSized() const {
    return _dim[0] != -1;
  }; // dim[0] == -1 flags "uninitialized"
  void SetDimensions(int row, int col) {
    _dim[0] = row;
    _dim[1] = col;
  };
  void GetDimensions(int &row, int &col) const {
    row = _dim[0];
    col = _dim[1];
  };
  void Clear() { _dim[0] = -1; }
...
/*
  ...
  Caveat: Only diagonal entries are stored. Any entrie not being "created"
          ...
          can be avoided only if the user is certain all entries will be
  created.
