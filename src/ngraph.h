...
template <class T, int FIXSZ = DEFAULT_VERTEXSIZE> class TinyVec {
protected:
  T _fixmem[FIXSZ];
  int _asz;
  T *_vec;

  // if not large enough, allocates more than requested and copies the old data
  // into new memory
  int _do_grow(int sz) {
    T *newvec = new T[sz + _asz]; // grow the memory block size
    for (int k = 0; k != _asz; ++k)
      newvec[k] = _vec[k];
    if (_vec != (T *)_fixmem)
      delete[] _vec;
    _vec = newvec;
...
public:
  ...
  */
  ...
  int StackDepth(void) const { return _current; }
...
public:
  vertex()
      : _id(-1), _type(UNDEFINED), _up(NULL), _down(NULL), _other(NULL),
        _flag(TCHK_OK), _injection(0.0), _sum_down(0.0), _sum_up(0.0) {}
  vertex(int id)
      : _type(UNDEFINED), _up(NULL), _down(NULL), _other(NULL), _flag(TCHK_OK),
        _injection(0.0), _sum_down(0.0), _sum_up(0.0) {
    _id = id;
  }
...
protected:
  int _id;
  unsigned int
      _strahler; // we can compute strahler number, but it's not done yet
  vertex *_from;
  vertex *_to;
...
public:
  ...
  edge(int id) : _strahler(0), _from(NULL), _to(NULL), _len(0.0) { _id = id; }
...
/*
   ...
   Each node should have a unique id, which corresponds to the order they are
...
public:
  ...
  ~ngraph() {
    ...
    for (jj = 0; jj < _num_edges; jj++) {
      if (_edges[jj])
        delete _edges[jj];
    }
    for (jj = 0; jj < _num_junctions; jj++) {
      if (_junctions[jj])
        delete _junctions[jj];
    }
  }
  ...
  double GetQ(int j_idx) {
    if (!_chk_ok)
      return -1.0; // anything negative is bad
