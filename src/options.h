...
private:
  ...
  double _k_ratio;   // factor to emphasis ratio between internal and connection
                     // nodes
...
public:
  ...
  // void  SetChkSum(char *s) { assert(s!=NULL); strncpy(_chksum, s, 8);
  // _chksum[8]=0;} char* Epoch()  { return _epoch; } void  SetEpoch(const char
  // *s) { assert(s!=NULL); strncpy(_epoch, s, 128); _epoch[127]=0; }
};
...
public:
  Stats()
      : _n_nodes(0), _n_segs(0), _n_juncs(0), _n_qsrcs(0), _n_lsrcs(0),
        _n_bnds(0) {
    _ss_steps = 0;
    _dy_steps = 0;
  ...
  void Add_ss_step(double t, double step, int niter, int isconverged) {
    ...
    _steady_stats[(int)_ss_steps]._time =
        t; // down converting, could be dangerous
  ...
  void SetSSFile(char *s) {
    assert(s != NULL);
    strncpy(_ssfile, s, 512);
    _ssfile[511] = 0;
  }
  ...
  void SetInFile(const char *s) {
    assert(s != NULL);
    strncpy(_in_file, s, 128);
    _in_file[127] = 0;
  }

  char *Epoch() { return _epoch; }
  void SetEpoch(const char *s) {
    assert(s != NULL);
    strncpy(_epoch, s, 128);
    _epoch[127] = 0;
  }
};
