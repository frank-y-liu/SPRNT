...
int XY_XSection::Build(int num, double *xx, double *yy, xs_trans *m) {
  ...
  for (jj = 0; jj < N; jj++) {
    printf("=6= %.5e %.5e %.5e %.5e\n", jj * st, _per_sp.EvalFun(jj * st),
           _y_sp.EvalFun(jj * st), _w_sp.EvalFun(jj * st));
  }
...
int XY_XSection::Build(int num_sample, double *aa, double *pp, double *yy,
                       double *ww) {

  if (num_sample < 5) {
    fprintf(stderr,
            "INTRINSIC cross section requires at least 5 data points for "
            "x-section %d.\n",
            _id);
    return (-1);
  }
  ...
  for (jj = 0; jj < 1000; jj++) {
    printf("=6= %.5e %.5e %.5e %.5e\n", jj * st, _w_sp.EvalFun(jj * st),
           _per_sp.EvalFun(jj * st), _y_sp.EvalFun(jj * st));
  }
#endif
...
double XY_XSection::GetEqFriction(double a) {
  ...
  if (a > _aa_bf) { // we only print this error message in this function!
    if (OPT.DebugLevel() >= 2)
      fprintf(stderr,
              "XY_XSection (%d) have reached bankfull area %.2e (%.2e)\n", _id,
              a, _aa_bf);
    ...
    if (OPT.DebugLevel() >= 2)
      fprintf(stderr, "XY_XSection (%d) has small area %.2e (%.2e)\n", _id, a,
              _aa_min);
    p = _per_sp.EvalFun(a);
  } else {
    p = _per_sp.EvalFun(a);
...
double XY_XSection::GetAbyDepth(double h) {
  ...
  if (h < h0) { // too small, return min
    ...
    for (jj = 0; jj < maxiter; jj++) {
      // will return crappy results if the iteration doesn't converge within the
      // maxiter
