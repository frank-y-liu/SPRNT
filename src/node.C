...
int ArSrcEqn::Evaluate(Node **n, double *X, double *Xp, double *RHS, SparseMatrix *M) {
  int where = n[_nidx]->AIdx();
...
int ArSrcEqn::EvaluateRHS(double t, double dt, Node **n, double *X, double *Xp, double *RHS) {
  int where = n[_nidx]->AIdx();
...
int ArSrcEqn::Evaluate(double t, double dt, Node **n, double *X, double *Xp, double *RHS, SparseMatrix *M) {
  int where = n[_nidx]->AIdx();
  double v = _S->Evaluate(t);
...
int ArSrcEqn::EvaluateJac(double t, double dt, Node **n, double *X, double *Xp, SparseMatrix *M) {
  // obviously not yet implemented. ditto for others
  return OK;
}

void ArSrcEqn::PrintValues(FILE *F, double t, double dt, Node **n, double *X, double *Xp, double *RHS) {
  int where = n[_nidx]->AIdx();
  fprintf(F,"ASRC: (%4d %4d)->%6.2e || (%4d)->%+6.2e\n", _row, where, 1.0, _row, RHS[_row] );
}
...
int ArDepEqn::EvaluateJac(double t, double dt, Node **n, double *X, double *Xp, SparseMatrix *M) {
  return OK;
}
...
int ArDepEqn::Evaluate(Node **n, double *X, double *Xp, double *RHS, SparseMatrix *M) {
  int idx1 = n[_nidx]->AIdx();
  int idx2 = n[_dep_idx]->AIdx();
...
int ArDepEqn::EvaluateRHS(double t, double dt, Node **n, double *X, double *Xp, double *RHS) {
  int idx1 = n[_nidx]->AIdx();
  int idx2 = n[_dep_idx]->AIdx();
...
int ArDepEqn::Evaluate(double t, double dt, Node **n, double *X, double *Xp, double *RHS, SparseMatrix *M) {
  int idx1 = n[_nidx]->AIdx();
  int idx2 = n[_dep_idx]->AIdx();
...
void ArDepEqn::PrintValues(FILE *F, double t, double dt, Node **n, double *X, double *Xp, double *RHS) {
  int idx1 = n[_nidx]->AIdx();
  int idx2 = n[_dep_idx]->AIdx();

  fprintf(F,"ADEP: (%4d %4d)->%6.2e (%4d %4d)->%6.2e || (4%d)->%+6.2e\n", 
	  _row, idx1, 1.0, _row, idx2, -_ratio, _row, RHS[_row] );
}
...
int MasStvEqn::Evaluate(Node **n, double *X, double *Xp, double *RHS, SparseMatrix *M) {
...
int MasStvEqn::EvaluateRHS(double t, double dt, Node **n, double *X, double *Xp, double *RHS) {
  ...
  RHS[_row]= X[idx_up_a] + X[idx_dn_a] - Xp[idx_up_a] - Xp[idx_dn_a] + lambda2*(X[idx_dn_q]-X[idx_up_q]);
...
int MasStvEqn::Evaluate(double t, double dt, Node **n, double *X, double *Xp, double *RHS, SparseMatrix *M) {
  double lambda2 = 2.0*dt/_dx;
  double pv;
  int idx_up_a = n[_up_idx]->AIdx();
  int idx_up_q = n[_up_idx]->QIdx();
...
int MasStvEqn::EvaluateJac(double t, double dt, Node **n, double *X, double *Xp, SparseMatrix *M) {
  return OK;
}

void MasStvEqn::PrintValues(FILE *F, double t, double dt, Node **n, double *X, double *Xp, double *RHS) {
  double lambda2 = 2.0*dt/_dx;
  int idx_up_a = n[_up_idx]->AIdx();
  int idx_up_q = n[_up_idx]->QIdx();
  int idx_dn_a = n[_dn_idx]->AIdx();
  ...
  fprintf(F,"MASV: (%4d %4d)=%6.2e (%4d %4d)=%6.2e (%4d %4d)=%6.2e (%4d %4d)=%6.2e || (%4d)->%+6.2e\n", 
	  _row, idx_up_a, 1.0, _row, idx_dn_a, 1.0, _row, idx_up_q, -lambda2, _row, idx_dn_q, lambda2, _row,RHS[_row] );
...
int QrSrcEqn::Evaluate(Node **n, double *X, double *Xp, double *RHS, SparseMatrix *M) {
...
int QrSrcEqn::EvaluateRHS(double t, double dt, Node **n, double *X, double *Xp, double *RHS) {
  int where = n[_nidx]->QIdx();
  double v = _S->Evaluate(t);
...
int QrSrcEqn::Evaluate(double t, double dt, Node **n, double *X, double *Xp, double *RHS, SparseMatrix *M) {
  int where = n[_nidx]->QIdx();
  double v = _S->Evaluate(t);
...
int QrSrcEqn::EvaluateJac(double t, double dt, Node **n, double *X, double *Xp, SparseMatrix *M) {
  return OK;
}

void QrSrcEqn::PrintValues(FILE *F, double t, double dt, Node **n, double *X, double *Xp, double *RHS) {
  int where = n[_nidx]->QIdx();
...
int QrDepEqn::AddDep(int sz, int* indices) {
  ...
  _deps.size(sz);
  for (int j=0; j<sz; j++) _deps[j]=indices[j];
  _num_deps = sz;
  return sz;
}
...
int QrDepEqn::Evaluate(Node **n, double *X, double *Xp, double *RHS, SparseMatrix *M) {
...
int QrDepEqn::EvaluateRHS(double t, double dt, Node **n, double *X, double *Xp, double *RHS) {
  int idx1 = n[_nidx]->QIdx();
  int idx2;
  double v=X[idx1];
  double pv;
...
int QrDepEqn::Evaluate(double t, double dt, Node **n, double *X, double *Xp, double *RHS, SparseMatrix *M) {
  int idx1 = n[_nidx]->QIdx();
  int idx2;
  double v=X[idx1];
  double pv;
...
int QrDepEqn::EvaluateJac(double t, double dt, Node **n, double *X, double *Xp, SparseMatrix *M) {
  // to be implemented, see Evaluate() method
  return OK;
}

void QrDepEqn::PrintValues(FILE *F, double t, double dt, Node **n, double *X, double *Xp, double *RHS) {
  int idx1 = n[_nidx]->QIdx();
  int idx2,j;
...
int MomStvEqn::Evaluate(Node **n, double *X, double *Xp, double *RHS, SparseMatrix *M) {
  ...
  if ( OPT.HT() > 0 && dn_a < 2.0*OPT.HT() ) {
    ...
    if (dn_n_corr<10) dn_n_corr = 1 + (log(1+exp(12.0*(dn_n_corr-1))))/12.0;
    if (dn_n_corr>20) dn_n_corr = 20;
    if ( OPT.DebugLevel()  >= 2 ) {
      fprintf(stdout,"   HT invoked for a=%.2e, correction = %.2e\n", dn_a, dn_n_corr); 
    }
  }
...
int MomStvEqn::EvaluateJac(double t, double dt, Node **n, double *X, double *Xp, SparseMatrix *M) {
  return OK;
}
...
int MomStvEqn::EvaluateRHS(double t, double dt, Node **n, double *X, double *Xp, double *RHS) {
  ...
  RHS[_row]= ff*(X[idx_dn_q] + X[idx_up_q] - Xp[idx_dn_q] - Xp[idx_up_q])
    + lambda2*(  ff*OPT.Beta()* (Qdnsqdiva - Qupsqdiva) + 
		 OPT.Gravity()*(X[idx_dn_a]+X[idx_up_a])*(  DN->GetDepth(X[idx_dn_a])
							   -UP->GetDepth(X[idx_up_a])) 
		)
...
// If SUPPRESSOR on -> Use LPI from Fread (1986)
  ...
  double dmdaup = lambda2*(ff*OPT.Beta()*Qupsqdiva/(X[idx_up_a]) 
			   + OPT.Gravity()*(DN->GetDepth(X[idx_dn_a]) - UP->GetDepth(X[idx_up_a])
					    - (X[idx_up_a]+X[idx_dn_a])*UP->GetDepthdA(X[idx_up_a])   )) 
    - gdt*UP->GetSR()
    + gdt*UP->Nsq()*up_n_corr*Qupsq* up_fric_da;

  double dmdadn = lambda2*(-ff*OPT.Beta()*Qdnsqdiva/(X[idx_dn_a])
			   + OPT.Gravity()*(DN->GetDepth(X[idx_dn_a]) - UP->GetDepth(X[idx_up_a])
					    + (X[idx_up_a]+X[idx_dn_a])*DN->GetDepthdA(X[idx_dn_a]) ) )
    - gdt*DN->GetSR()
...
// this function need to be synchronized with the evaluation code! it is becoming obsolete!!!
void MomStvEqn::PrintValues(FILE *F, double t, double dt, Node **n, double *X, double *Xp, double *RHS) {
  double lambda2 = 2.0*dt/_dx;
  double gdt = OPT.Gravity()*dt;
  Node *UP = n[_up_idx];
  Node *DN = n[_dn_idx];
  ...
  double dmdqup = 1.0 - 2.0*lambda2*OPT.Beta()*X[idx_up_q]/X[idx_up_a] + 2.0*gdt*UP->Nsq()*Efup*ABS(X[idx_up_q]);
  double dmdqdn = 1.0 + 2.0*lambda2*OPT.Beta()*X[idx_dn_q]/X[idx_dn_a] + 2.0*gdt*DN->Nsq()*Efdn*ABS(X[idx_dn_q]);

  double dmdaup = lambda2*(OPT.Beta()*Qupsqdiva/X[idx_up_a] - OPT.Gravity() * UP->XS()->GetCentroiddA(X[idx_up_a]))
    - gdt*UP->GetSR()
    + gdt*UP->Nsq()*Qupsq*UP->XS()->GetEqFrictiondA(X[idx_up_a]);

  double dmdadn = lambda2*(-OPT.Beta()*Qdnsqdiva/X[idx_dn_a] + OPT.Gravity() * DN->XS()->GetCentroiddA(X[idx_dn_a]))
    - gdt*DN->GetSR()
  ...
  fprintf(F,"** Qup=%6.2e Aup=%6.2e Qdown=%6.2e Adown=%6.2e\n",X[idx_up_q],
	  X[idx_up_a], X[idx_dn_q], X[idx_dn_a]);
  fprintf(F,"MOMV: (%4d %4d)=%6.2e (%4d %4d)=%6.2e (%4d %4d)=%6.2e (%4d %4d)=%6.2e || (%4d)=%+6.2e\n",
	  _row, UP->QIdx(), dmdqup, _row, DN->QIdx(), dmdqdn, _row, UP->AIdx(), dmdaup, _row, DN->AIdx(), dmdadn,
	  _row, RHS[_row]);
}
