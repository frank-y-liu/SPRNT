...
int PWL::CheckTime(int sz, double *t) {
  ...
  if ((t[sz - 1] - t[0]) < OPT.Epsilon())
    thresh = OPT.Epsilon();
  if (sz < 2)
    return (-1);

  for (int jj = 1; jj < sz; jj++) {
    if ((t[jj] - t[jj - 1]) < thresh)
      return (-1);
  }
  return 0;
}

// make sure the time sequence is monotonically increasing, also calculate the
// slopes
int PWL::_check() {
...
/* the purpose of _p1 and _p2 is to speed up the evaluation speed
   since the evaluation time is most likely sequential, we store the two
   previous "knots" so that we don't have to search for the "knots" every time.
*/
...
double PWL::Evaluate(double t) {
  ...
  if (!(_T[_p1] < t && _T[_p2] > t))
    _lin_search(t); // and modify _p1, _p2
...
double PWL::NextT(double t) {
  if (!(_T[_p1] < t && _T[_p2] > t))
    _lin_search(t);
  if (_T[_p2] - t <
      TUNIT) { // if we are within striking limit of the next point, we
    // move over
    if (_p2 == _size - 1)
      return (FLT_MAX);
    return (_T[_p2 + 1]);
...
// if there is time overlap, the new value will overwrite the existing time
// series if there is no overlap, the new series is concatenated to the existing
// series
