...
public:
  ...
  Waveforms(int num_nodes)
      : _num_banks(UNK_STORE), _height(num_nodes), _time_idx(-1),
        _data(UNK_STORE, num_nodes, DFT_STORE_LEN) {
    _time.size(DFT_STORE_LEN);
  }

  // copy constructor, copy everything!
  Waveforms(const Waveforms &other)
      : _num_banks(other._num_banks), _height(other._height),
        _time_idx(other._time_idx), _data(other._data) {
    // except growvec doesn't have a copy constructor
    _time.size(_time_idx + 1);
    for (int jj = 0; jj <= _time_idx; jj++)
      _time[jj] = other._time.getVal(jj);
  }
  ...
  int TimeSeq(double *target) {
    for (int jj = 0; jj <= _time_idx + 1; jj++)
      target[jj] = _time.getVal(jj);
    return (_time_idx + 1);
  }
