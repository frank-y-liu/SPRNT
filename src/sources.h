...
public:
  ...
  double FinalT() const {
    return (_T.getVal(_size - 1));
  } // obvious this is only meaningful after
    // Finalize()
  ...
  /* Method2:
     ...
     if (rc<0) error!
     this method can be used to overwrite or concatenate to the existing time
     series example: t1=[0 1 2 3], y1=[....] t2=[4 5 6],   y2=[....] it's
     perfectly fine to do: Assign(4, t1,y1); Assign(3, t2,y2); which has the
     same net effect as calling: Assign(7,[0 1 2 3 4 5 6],[.....]);
  ...
  */
  ...
  /* if t>TEnd(), the time series value will be assumed to be
   * piece-wise-constant */
  double Evaluate(double t); // evaluation given the time point

  double NextT(double t); // return the next time point in the queue
  int GetNextT(double cur,
               double &nxt); // better way to find the next time point
};
