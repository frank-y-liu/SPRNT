...
public:
  ...
  virtual void GetCentroid(double a, double &cen, double &cenda) {
  } // they might be
    // cheaper
  ...
  // finite difference method to check derivatives, they are not intended for
  // real code
...
public:
  R_XSection(int id, double b0) : XSection(id) {
    assert(b0 > 0);
    _b0 = b0;
  }
  ~R_XSection(){};
