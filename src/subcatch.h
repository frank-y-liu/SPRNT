...
protected:
  ...
  int NonLinearStep(double t, double dt, int jac_num, int max_iter, double tol,
                    int do_bounding, int &num_iter, double &minnorm,
                    double &maxnorm, double &tdif, FILE *F = 0);
...
public:
  Subcatchment(int cid)
      : _cid(cid), _num_nodes(0), _num_eqns(0), _num_src(0), _num_superc(0) {
    _s = NULL;
    _max_froude = 0.0;
    _t = 0.0;
    _dt = 0.0;
  ...
  void TopoPrintErrMsg(FILE *F, const char *h, char **table) {
    _G.PrintErrorMsg(F, h, table);
  }

  /* methods to construct the subcatchment, returns the node index*/
  node_id MakeNode(int id, float s0, float n, double x, double y, float q0,
                   float a0, float z0, float h0, XsecType tp,
                   double b0); // RECT
  node_id MakeNode(int id, float s0, float n, double x, double y, float q0,
                   float a0, float z0, float h0, XsecType tp, double b0,
                   double s); // TRAP
  node_id MakeNode(int id, float s0, float n, double x, double y, float q0,
                   float a0, float z0, float h0, XsecType tp, int sec_num,
                   double *xsecx, double *xsecy); // XY
  node_id MakeNode(int id, float s0, float n, double x, double y, float q0,
                   float a0, float z0, float h0, XsecType tp, int num_pt,
                   double *aa, double *pp, double *yy, double *ww); // INTRINSIC

  // determine the total number of nodes, and assign the indices to the X vector
  int AssignNodes();

  /* make time-varying sources, so that they can be later used to build
     equations since they are "dynamic", they can also be updated effectively
     only PWL sources are supported */
  // returns the index, return -1 if error
  source_id MakeSource(int sz, double *t, double *y, double t_scale = 1.0,
                       double y_scal = 1.0);
  // update, returns <0 if error
  int UpdateSource(source_id which, int sz, double *t, double *y,
                   double t_scale = 1.0, double y_scale = 1.0);
  // retrieval
  Source *GetSource(source_id which) {
  ...
  Node *GetNode(int which) {
    assert(which < _num_nodes);
    return _NS[which];
  }
  Equation *GetEquation(int which) {
    assert(which < _num_eqns);
    return _EQ[which];
  }
  ...
  int UnsteadySolve(double final_t, int jac_num, int max_iter, double tol,
                    Waveforms *WV, sptFile OUT, int what2print, int pstart,
                    char **N);

  /* printing */
  void PrintOnDemand(sptFile F, double tnow, int tstart, int what = 0,
                     char **NAMES = NULL);
  void PrintHeader(sptFile F, int steadyonly = 0);

  /* methods for debugging */
  void PrintToFile(FILE *F, int converged_x = 1);
  void PrintQToFile(FILE *F,
                    int partial_list_only = 1); // filter out those w/ zero xy
  ...
  inline double GetQ(int nidx) {
    assert(nidx < _num_nodes);
    return (_Xp[2 * nidx]);
  }
  inline double GetA(int nidx) {
    assert(nidx < _num_nodes);
    return (_Xp[2 * nidx + 1]);
  }
  ...
  void PrintEvaluationValues(double t, double dt, FILE *F,
                             const char *separator = "NoName");
