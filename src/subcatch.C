...
/******************************************************************************
  methods related to the subcathment class. here is where the system is
explicitly solved
******************************************************************************/
...
void Subcatchment::PrintToFile(FILE *F, int previous_x) {
  ...
  if (previous_x == 1) {
    for (int i = 0; i < _num_eqns; i += 2)
      fprintf(F, "%5d  %10.5e %10.5e\n", i / 2, _X[i], _X[i + 1]);
  } else {
    for (int i = 0; i < _num_eqns; i += 2)
      fprintf(F, "%5d  %10.5e %10.5e\n", i / 2, _Xp[i], _Xp[i + 1]);
  }
...
/* save steady state to a file
   ...
   but it also has the benefit of capable of having a hot restart for unsteady
   case
 */
...
int Subcatchment::LoadSteadyStateFromFile(FILE *F, int cmp_chksum) {
  ...
  if ((fgets(buf, 512, F)) == NULL) {
    fprintf(stderr,
            "[WW]: Unable to read content from steadystate file %s. Stored "
            "steady state ignored!\n",
            STAT.SSFile());
    return WARNING;
  }
  ...
  if (sz != _num_eqns) {
    fprintf(stderr,
            "[WW]: Size mismatch in steadystate file %s. Stored steady state "
            "ignored!\n",
            STAT.SSFile());
    return WARNING;
  }
  ...
  if (cmp_chksum == 1) {
    if (strncmp(chksum, STAT.ChkSum(), 8) != 0) {
      fprintf(stderr,
              "[WW]: Signature mismatch found in steadystate file %s. Stored "
              "steady state ignored!\n",
              STAT.SSFile());
      return WARNING;
    }
...
node_id Subcatchment::MakeNode(int id, float s0, float n, double x, double y,
                               float q0, float a0, float z0, float h0,
...
node_id Subcatchment::MakeNode(int id, float s0, float n, double x, double y,
                               float q0, float a0, float z0, float h0,
                               XsecType tp, double b0, double s) {
  T_XSection *tx;
  Node *node;
...
node_id Subcatchment::MakeNode(int id, float s0, float n, double x, double y,
                               float q0, float a0, float z0, float h0,
                               XsecType tp, int sec_num, double *xsecx,
                               double *xsecy) {
  XY_XSection *tx;
  Node *node;
  char buf[512];
...
node_id Subcatchment::MakeNode(int id, float s0, float n, double x, double y,
                               float q0, float a0, float z0, float h0,
                               XsecType tp, int num_pts, double *aa, double *pp,
                               double *yy, double *ww) {
 ...
 */
source_id Subcatchment::MakeSource(int sz, double *t, double *y, double t_scale,
                                   double y_scale) {
  PWL *ns = new PWL;
  int rc;
...
// rc < 0 if error
int Subcatchment::UpdateSource(source_id which, int sz, double *t, double *y,
                               double t_scale, double y_scale) {
  assert(which < _num_src);

  if (_t > t[0])
    return (-1); // we only allow update for the future
  if (PWL::CheckTime(sz, t) < 0)
    return (-1);
...
// returns the number of equations added
int Subcatchment::MakeDepEquation(int dn_idx, int up_sz, int *up_idx,
                                  double *ratios) {
...
// add st venant equations, need to call it twice, one for momentum and one for
// mass returns the number of equation added (e.g., 1 means good )
int Subcatchment::MakeStvEquation(int up_idx, int dn_idx, double dx,
                                  Source *pp) {
  MasStvEqn *E1;
  MomStvEqn *E2;
  ...
  if (pp) {
    ...
    pp->SetParent(
        up_idx); // this would prevent the src to be used by anybody else

    // we only insert lateral flow if we do NOT use spin up
    if (OPT.SpinUpTime() <= 0)
      _G.InsertQ(up_idx, pp->Evaluate(0));
  }
...
int Subcatchment::MakeSolver(int mx_sz) {
  ...
  if (_num_eqns - 2 * _num_nodes != 0) {
    printf("The numbers of equations and unknowns are not the same (#eqn=%d, "
           "#unk=%d). Bummer!\n",
...
// calculate froude number, also returns the ideal step size based on the
// average of flow velocity and celerity
double Subcatchment::CalFroude() {
  ...
  for (j = 0; j < _num_nodes; j++)
    _workb[j] = _X[_NS[j]->AIdx()] / _NS[j]->XS()->GetWidth(_X[_NS[j]->AIdx()]);
  for (j = 0; j < _num_nodes; j++)
    _workb[j] = sqrt(_workb[j]);
  FORTRAN(dscal)(&_num_nodes, &sqg, &_workb[0], &onei);

  // calculate the froude number
  for (j = 0; j < _num_nodes; j++)
    _Froude[j] = _work[2 * j] / _workb[j];
...
// Kinematic routing based on Q0, make sure they are populated before calling
// this function whatever in A will be ignored
void Subcatchment::KinematicEstimate(FILE *F, char **NAMES) {
  ...
  if (OPT.UseMetric() == 1) {
    ...
    for (jj = 0; jj < _num_nodes; jj++) {
      ...
      if (!NAMES)
        fprintf(F, "%9d ", _NS[jj]->Id());
      else
        fprintf(F, "%9s ", NAMES[_NS[jj]->Id()]);
...
int Subcatchment::InitSolutions(void) {
  ...
  for (int j = 0; j < _num_nodes; j++)
    _NS[j]->SetInitValues((double *)_Xp);
...
void Subcatchment::PrintEvaluationValues(double t, double dt, FILE *F,
                                         const char *name) {
  int j;
  for (j = 0; j < _num_eqns; j++) {
    _EQ[j]->PrintValues(F, t, dt, (Node **)_NS, (double *)_X, (double *)_Xp,
                        (double *)_RHS);
  }
}
...
int Subcatchment::Evaluate(double t, double dt) {
  ...
  if (dt > 0) {
    for (int j = 0; j < _num_eqns; j++) {
      _EQ[j]->Evaluate(t, dt, (Node **)_NS, (double *)_X, (double *)_Xp,
                       (double *)_RHS, &_M);
    }
  } else {
    for (int j = 0; j < _num_eqns; j++) {
      _EQ[j]->Evaluate((Node **)_NS, (double *)_X, (double *)_Xp,
                       (double *)_RHS, &_M);
    }
  }
  _M.SetDimensions(_num_eqns, _num_eqns);
...
int Subcatchment::EvaluateRHS(double t, double dt) {
  if (dt > 0) {
    for (int j = 0; j < _num_eqns; j++) {
      _EQ[j]->EvaluateRHS(t, dt, (Node **)_NS, (double *)_X, (double *)_Xp,
                          (double *)_RHS);
    }
  } else {
    for (int j = 0; j < _num_eqns; j++) {
      _EQ[j]->EvaluateRHS((Node **)_NS, (double *)_X, (double *)_Xp,
                          (double *)_RHS);
    }
  }
  _M.SetDimensions(_num_eqns, _num_eqns);
...
double Subcatchment::Accept(double t, int needed, double &qdiff,
                            double &adiff) {
  const int twoi = 2;
  const double negone = -1.0;
  const int onei = 1;
  ...
  if (needed > 0) {
    ...
    if (needed >= 2) {
      qdiff = FORTRAN(dnrm2)(&_num_nodes, &_work[0],
                             &twoi); // skip 2, the number should be half!
  ...
  memcpy(&_Xtm1[0], &_Xp[0],
         _num_eqns * sizeof(double)); // save a copy for printing
  _tm1 = _t;
  ...
  if (OPT.DebugLevel() >= 2) {
    ...
    for (int j = 0; j < _num_nodes; j++) {
      ...
      fprintf(stderr,
              "      %5d Q( %11.4e ) A( %11.4e ) Y( %11.4e ) EqFrc( %11.4e ) "
              "s0( %11.4e ) sf( %11.4e )\n",
              _NS[j]->Id(), qq, aa, _NS[j]->GetDepth(aa), eqf, _NS[j]->GetSR(),
              sf);
    }
  }
  return tdiff;
...
inline double lin_interp(double t1, double x1, double t2, double x2,
                         double tt) {
...
void Subcatchment::PrintHeader(sptFile F, int steadyonly) {
  ...
  sptFprintf(F, "*** SPRNt Results. Netlist: \"%s\" Epoch: \"%s\"",
             STAT.InFile(), STAT.Epoch());
  if (!steadyonly) {
    sptFprintf(F, " Reqested Tstop: ");

    if ((((int)OPT.StopTime()) / 3600) > 0) {
      sptFprintf(F, "%d hr %d min\n", ((int)OPT.StopTime()) / 3600,
                 (((int)OPT.StopTime()) / 60) % 60);
    } else {
      sptFprintf(F, "%d min\n", ((int)OPT.StopTime()) / 60);
    }
  } else {
...
void Subcatchment::PrintOnDemand(sptFile F, double tnow, int pstart, int what,
                                 char **NAMES) {
  double fq, fa, fd;
  if (OPT.UseMetric() == 1) {
    fq = 1.0;
  ...
  if (!(what & (PRT_Q | PRT_A | PRT_D | PRT_Z)))
    return;
  ...
  if (interval == 0) { // we print whatever we have if interval is 0
    ...
    if (OPT.UseMetric() == 1) {
      ...
      if (what & PRT_Q)
        sptFprintf(F, " flow(m3/s)");
      if (what & PRT_A)
        sptFprintf(F, " wet_a(m2)");
      if (what & PRT_D)
        sptFprintf(F, " depth(m)");
      if (what & PRT_Z)
        sptFprintf(F, " surf_elev(m)");
      if (what & PRT_XY)
        sptFprintf(F, " xy-coordinates");
      sptFprintf(F, "\n");
    } else {
      sptFprintf(F, "*** id time(min)");
      if (what & PRT_Q)
        sptFprintf(F, " flow(ft3/s)");
      if (what & PRT_A)
        sptFprintf(F, " wet_a(ft2)");
      if (what & PRT_D)
        sptFprintf(F, " depth(ft)");
      if (what & PRT_Z)
        sptFprintf(F, " surf_elev(ft)");
      if (what & PRT_XY)
        sptFprintf(F, " xy-coordinates");
      sptFprintf(F, "\n");
    }
    ...
    for (int jj = 0; jj < _num_nodes; jj++) {
      if (!NAMES)
        sptFprintf(F, "%d %6.2f", _NS[jj]->Id(), tnow / 60.0);
      else
        sptFprintf(F, "%s %6.2f", NAMES[_NS[jj]->Id()], tnow / 60.0);
      if (what & PRT_Q)
        sptFprintf(F, PRT_FMT, _X[_NS[jj]->QIdx()] * fq);
      if (what & PRT_A)
        sptFprintf(F, PRT_FMT, _X[_NS[jj]->AIdx()] * fa);
      if (what & PRT_D)
        sptFprintf(F, PRT_FMT, _Depth[jj] * fd);
      if (what & PRT_Z)
        sptFprintf(F, PRT_FMT, _Elevation[jj] * fd);
      if (what & PRT_XY)
        sptFprintf(F, PRT_XY_FMT, _NS[jj]->X(), _NS[jj]->Y());
      sptFprintf(F, "\n");
    }
    ...
    if (prev_t < 0 && pstart == 0) {
      ...
      for (int jj = 0; jj < _num_nodes; jj++) {
        if (!NAMES)
          sptFprintf(F, "%d %d", _NS[jj]->Id(), 0);
        else
          sptFprintf(F, "%s %d", NAMES[_NS[jj]->Id()], 0);
        if (what & PRT_Q)
          sptFprintf(F, PRT_FMT, _X[_NS[jj]->QIdx()] * fq);
        if (what & PRT_A)
          sptFprintf(F, PRT_FMT, _X[_NS[jj]->AIdx()] * fa);
      ...
      while (prev_t + interval <= tnowmin) {
        ...
        for (int jj = 0; jj < _num_nodes; jj++) {
          if (!NAMES)
            sptFprintf(F, "%d %d", _NS[jj]->Id(), prev_t + interval);
          else
            sptFprintf(F, "%s %d", NAMES[_NS[jj]->Id()], prev_t + interval);
          // in order for interpolation, we recompute depth and elevation at the
          // previous time point on the fly. Note that "tprt" is given in
          // minutes
          if (what & PRT_Q)
            sptFprintf(F, PRT_FMT,
                       lin_interp(_tm1, _Xtm1[_NS[jj]->QIdx()], _t,
                                  _X[_NS[jj]->QIdx()], tprt * 60.0) *
                           fq);
          if (what & PRT_A)
            sptFprintf(F, PRT_FMT,
                       lin_interp(_tm1, _Xtm1[_NS[jj]->AIdx()], _t,
                                  _X[_NS[jj]->AIdx()], tprt * 60.0) *
                           fa);
          if (what & PRT_D)
            sptFprintf(
                F, PRT_FMT,
                lin_interp(_tm1,
                           _NS[jj]->GetAbsoluteDepth(_Xtm1[_NS[jj]->AIdx()]),
                           _t, _Depth[jj], tprt * 60.0) *
                    fd);
          if (what & PRT_Z)
            sptFprintf(F, PRT_FMT,
                       lin_interp(_tm1,
                                  _NS[jj]->GetElevation(_Xtm1[_NS[jj]->AIdx()]),
                                  _t, _Elevation[jj], tprt * 60.0) *
                           fd);
...
/*
   steady solve, this version uses forward Euler method to come to a close
   enough steady state before using the pseudo dynamic method

   it relies on the completed tchk results: Q at each node, and A at the
   outpouring node stored

   assuming Q is constant, we can compute the derivative dA/dx from steady-state
   SV. then use explicite Euler's scheme to compute A from downstream to
   upstream.
...
int Subcatchment::SteadySolve(int jac_num, int max_iter, double tol,
  ...
  if (_G.GetNumRoots() != 1) {
    fprintf(stdout,
            "[EE] Bummer: requires one (and only one) root node. Found %d\n",
            _G.GetNumRoots());
    return (-1);
  }
  ...
  // no need to do DFS traversal, but use stack so that we can handle branches
  // nicely
  ...
  // we don't do anything for the root node, since A is already given, just copy
  // the values
  pn->Q0() = qq;
  pn->A0() = aa;
  ...
  while (1) {
    ...
    status =
        _G.GetUpstreamLength(cur, len); // whether we are at branch point or not
    if (status == 0) {
      nxt = _G.GetUpstreamNode(cur);
      ST.Push(nxt);
    } else {
  ...
  if (use_acc == 1) {
    ...
    rc = NonLinearStep(0.0, -1.0, jac_num, max_iter, tol, dobounding, num_iter,
                       minnorm, maxnorm, tdiff, F);
...
int Subcatchment::SteadySolve(double init_dt, int jac_num, int max_iter,
                              double tol) {
  ...
  if (OPT.SpinUpTime() > 0) {
    ...
    while (pt <= ((double)OPT.SpinUpTime())) {
      ...
      rc = NonLinearStep(0.0, dt, jac_num, max_iter, tol, dobounding, num_iter,
                         minnorm, maxnorm, tdif, F);
      t_num_iter += num_iter;
      dc_iter++;
      ...
      if (OPT.DebugLevel() >= 1) {
        CalSolStat();
        fprintf(stdout,
                "  S Iter w/ dt=%.2e %s, nxt_dt=%.2e, num_iter=%3d, "
                "max_norm=%.2e, tdif=%.2e, n_sc=%2d, max_f=%.3f\n",
                dt_used, rc < 0 ? "FAILED" : "passed", dt, num_iter, maxnorm,
                tdif, GetNumSuperC(), GetMaxFroude());
      }

      cnt++;
    }
    fprintf(stdout,
            "[II]: Spin-up completed for %d seconds, with %d steps and %d "
            "iterations.\n",
  ...
  fprintf(
      stdout,
      "[II]: SteadyState phase II converged in %d steps with %d iterations.\n",
      cnt, t_num_iter);
 ...
 */
int Subcatchment::UnsteadySolve(double final_t, int jac_num, int max_iter,
                                double tol, Waveforms *WV, sptFile OUT,
                                int what2print, int pstart, char **NAMES) {

  double current_t =
      _t; // we can call this many times, pick up t from previous runs
  int rc;
  double optdt, dtused;
  ...
  // print a header
  if (OUT != NULL)
    PrintHeader(OUT, 0); // header for unsteady
  ...
  while (1) {
    ...
    if (first_start) { // case1: first time enter restart, use previous
      ...
      if (dtused > optdt)
        dtused = optdt; // but we don't go too far
    }
    ...
    while (rc < 0) {
      ...
      if (rc < 0) {
        ...
        if (dtused < OPT.MinDT()) {
          fprintf(stderr,
                  "[EE] Bummer: time step too small at time point %.4e\n",
                  current_t);
          return ERROR;
        }
      }
    ...
    if (negcounter == 0 && niter_here < 14)
      poscounter++; // magic number
    else
      poscounter = 0;
    ...
    if (OPT.DebugLevel() >= 1) {
      ...
      fprintf(stdout,
              "  U time=%.4e (%6.2f out of %5.2f hrs), num_iter=%2d, "
              "falter=%d, dt=%.2e, n_sc=%3d, max_f=%5.2f, n_bk=%3d, n_mn=%3d\n",
              _t, _t / 3600, tf_hr, niter_here, negcounter, dtused,
              GetNumSuperC(), GetMaxFroude(), nbk, nmn);
    }

    // compute depth and elevation when needed
    if ((what2print & PRT_D) || (what2print & PRT_Z) || WV)
      ComputeDepthAndElevation(what2print);

    if (WV) { // not sure if we need to store the data
      int rc = WV->Advance();
      if (rc < 0) {
        fprintf(stdout, "[WW]: Error allocating waveform storage, no more "
                        "results stored.\n");
      } else {
  ...
  fprintf(stdout,
          "[II]: UnsteadyState finished w/ %d time points, %d iterations.\n",
          cnt, t_num_iter);
...
// return -1 if failed
int Subcatchment::NonLinearStep(double t, double dt, int jac_num, int max_iter,
                                double tol, int do_bounding, int &num_iter,
                                double &minn, double &maxn, double &tdiff,
                                FILE *F) {
  int rc = -1;
  int cnt;
  ...
  for (cnt = 0; cnt < max_iter; cnt++) {
    ...
    if (OPT.DebugLevel() >= 2) {
      fprintf(stdout,
              "    NL step %2d out of %2d, jac=%d, bnd=%1d, norm=%.2e\n", cnt,
              max_iter, jac_num, do_bounding, l2norm);
    }
    ...
    if (do_bounding) {
      ...
      FORTRAN(dscal)
      (&_num_nodes, &lowlim, &_Xlow[1], &twoi); // we bound A, hence start
                                                // from 1, and skip every
                                                // other entry
      FORTRAN(dscal)(&_num_nodes, &highlim, &_Xhigh[1], &twoi);
      for (int j = 1; j < _num_eqns; j += 2)
        _X[j] = _X[j] < _Xlow[j] ? _Xlow[j] : _X[j];
      for (int j = 1; j < _num_eqns; j += 2)
        _X[j] = _X[j] > _Xhigh[j] ? _Xhigh[j] : _X[j];
    }
    ...
    if (CheckMinA(OPT.MinA()) == -1)
      break;
  }

  if (rc == 0)
    tdiff = Accept(t, 1, dummy1, dummy2);
...
int Subcatchment::CheckBankFull() {
  int rc = 0;
  for (int j = 0; j < _num_nodes; j++)
    rc += _NS[j]->XS()->ReachedBankFull(_X[_NS[j]->AIdx()]);
  return rc;
}
int Subcatchment::CheckMinimalA() {
  int rc = 0;
  for (int j = 0; j < _num_nodes; j++)
    rc += _NS[j]->XS()->ReachedMinimalA(_X[_NS[j]->AIdx()]);
  return rc;
}
...
int Subcatchment::ComputeDepthAndElevation(int flags) {
  if (flags & PRT_D) {
    for (int jj = 0; jj < _num_nodes; jj++)
      _Depth[jj] = _NS[jj]->GetAbsoluteDepth(_X[_NS[jj]->AIdx()]);
  }
