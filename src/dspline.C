...
int DSpline::Construct(double *xx, double *yy) {
  ...
  FORTRAN(dbintk)(&xx[0], &yy[0], &_t[0], &_nx, &_k, &_bcoef[0], &_q[0], &_work[0]);
...
double DSpline::EvalFun(double x) {
  ...
  assert(x >= _xb && x <= _xe);
  y = FORTRAN(dbvalu)(&_t[0], &_bcoef[0], &_nx, &_k, &idx, &x, &_inbv, &_work[0]); 
  return y;
}
...
double DSpline::EvalDer(double x) {
  ...
  assert(x >= _xb && x <= _xe);
  dy = FORTRAN(dbvalu)(&_t[0], &_bcoef[0], &_nx, &_k, &idx, &x, &_inbv, &_work[0]); 
  return dy;
