...
void ngraph::Init(int max_node_idx, int max_edge_idx, int max_junc_idx) {
  ...
  for (jj=0; jj<max_node_idx;jj++) _nodes[jj] = NULL;
  for (jj=0; jj<max_edge_idx;jj++) _edges[jj] = NULL;
  for (jj=0; jj<max_junc_idx;jj++) _junctions[jj] = NULL;
}
...
void ngraph::PrintErrorMsg(FILE *F, const char* header, char** trtable) {
  int jj;
  for (jj=0; jj<_num_nodes; jj++) {
    if ( _nodes[jj] == NULL) continue; // just in case the id's are not continuous
...
int ngraph::GetRoots(int* arr) {
  ...
  for (int jj=0; jj<_num_nodes; jj++) {
    if ( _nodes[jj]->GetType() == ROOT ) arr[rc++] = _nodes[jj]->GetId();
  }
  return (rc);
}
...
void ngraph::PrintAll(FILE *F) {
  ...
  for (int jj=0; jj<_num_nodes; jj++) {
    fprintf(F,"%3d: [%4d]: %1d %1d %.2f %.2f %.2f\n", jj, _nodes[jj]->GetId(), 
	    ...
	    _nodes[jj]->Injection(), _nodes[jj]->SumDown(), _nodes[jj]->SumUp());
  }
