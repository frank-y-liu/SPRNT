...
protected:
  ...
  /*
     definition of the sr line:
       1. The distance from datum to the bathymetry bottom is z0 (usually
     positive)
       2. The distance from datum to "slope" line is zR (usually positive)
       3. The distance from "slope" line to bathymetry bottom is hR (can be pos
     or neg) always true:  zR + hR = z0
       4. The distance from "slope" line to free surface is ha, this is computed
     by solving SVE. Or y(A) + hR ( y(A) is computed from A, which is based on
     z0)
...
public:
  ...
  Node(unsigned int id, double sr, double n, XSection *xs, double x, double y,
       float q0, float a0, float zr, float hr, int p = 1)
...
public:
  ...
  virtual int EvaluateRHS(double t, double dt, Node **n, double *X, double *Xp,
                          double *RHS) {
    return 0;
  }
  virtual int EvaluateJac(double t, double dt, Node **n, double *X, double *Xp,
                          SparseMatrix *M) {
    return 0;
  }
  virtual int Evaluate(double t, double dt, Node **n, double *X, double *Xp,
                       double *RHS, SparseMatrix *M) {
    return 0;
  }

  virtual int EvaluateRHS(Node **n, double *X, double *Xp, double *RHS) {
    return 0;
  }
  virtual int Evaluate(Node **n, double *X, double *Xp, double *RHS,
                       SparseMatrix *M) {
    return 0;
  }

  // for debugging only, could be quite slow
  virtual void PrintValues(FILE *F, double t, double dt, Node **n, double *X,
...
public:
  ...
  int EvaluateRHS(double t, double dt, Node **n, double *X, double *Xp,
                  double *RHS);
  int EvaluateJac(double t, double dt, Node **n, double *X, double *Xp,
                  SparseMatrix *M);
  int Evaluate(double t, double dt, Node **n, double *X, double *Xp,
               double *RHS, SparseMatrix *M);
  ...
  void Assign(int nidx) {
    assert(nidx >= 0);
    _nidx = nidx;
  }
  void AddSource(Source *s) { _S = s; }

  void PrintValues(FILE *F, double t, double dt, Node **n, double *X,
                   double *Xp, double *RHS);
...
public:
  ...
  void Assign(int upidx, int dnidx) {
    assert(dnidx >= 0 && upidx >= 0);
    _up_idx = upidx;
    _dn_idx = dnidx;
  }
  ...
  static inline void SetSpinCoef(double c) {
    assert(c >= 0.0 && c <= 1.0);
    _spin_coef = c;
  }
