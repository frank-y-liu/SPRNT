...
/*
   ...
   Caveat: frequent small increase may cause performance degradation, use
   GrowVec instead
*/

template <class T, int FIXSZ = MAXFIXSZ> class FlexVec {
protected:
  T _fixmem[FIXSZ];
  int _asz;
  T *_vec;
  // if not large enough allocates more than requested and recopies the old data
  // into new memory
  int do_grow(int sz) {
...
public:
  ...
  T getVal(int i) const { return FlexVec<T, FIXSZ>::_vec[i]; }
};

/*
  GrowVecPair, a pair of GrowVec
 */
template <class T, int FIXSZ = MAXFIXSZ> class GrowVecPair {
protected:
