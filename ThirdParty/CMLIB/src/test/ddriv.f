...
1     FORMAT(I1,E10.0)
      ...
      CALL XSETUN(LUN)
      CALL XERMAX(1000)
C   TEST DDRIV1,DDRIV2,DDRIV3
      CALL DDRIQX(KPRINT,IPASS)
      ITEST=ITEST*IPASS
C
      IF(KPRINT.GE.1.AND.ITEST.NE.1) WRITE(LUN,2)
2     FORMAT(/' ***** WARNING -- AT LEAST ONE TEST FOR SUBLIBRARY DDRIV
     1  HAS FAILED ***** ')
      IF(KPRINT.GE.1.AND.ITEST.EQ.1) WRITE(LUN,3)
3     FORMAT(/' ----- SUBLIBRARY DDRIV PASSED ALL TESTS ----- ')
      END
      ...
      RETURN
      END
      SUBROUTINE DDRIQX(KPRINT,IPASS)
c
c     Quick Check Program
c
c   Part of DDRIV1,2,3 package, double precision version,
...
c
      ...
      common/unit/lun
      double precision eps,ewt(1),hmax,t,tout,work(264),y(3)
      integer iwork(24),kprint,ipass
      data n /3/, ntask /1/, eps /.001d0/, ewt(1) /.1d0/, ierror /3/,
     8     mint /1/, miter /5/, impl /0/, ml /2/, mu /2/, mxord /5/,
     8     hmax /1.d0/, lenw /264/, leniw /24/, nde /0/,
     8     mxstep /10/, iroot /0/
      ipass=1
      if(kprint.ge.2) then
      write(lun, '(// '' This program attempts to verify that all '',
     8  ''routines necessary for running'' / '' the ddriv package are'',
     ...
     8  ''that the test has actually passed.''  //)')
      ...
      tout = .001d0
      mstate = 1
      call ddriv2 (n, t, y, f, tout, mstate, iroot, eps, ewt,
     8             mint, work, lenw, iwork, leniw, g)
      ...
      nstate = 1
      tout = 1.d0
      call ddriv3 (n, t, y, f, nstate, tout, ntask, iroot, eps, ewt,
     ...
     8  nde, iflag)
      ...
      double precision function g (n, t, y, nroot)
      double precision y(*)
      g = 1.d0
      end
