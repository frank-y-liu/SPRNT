...
C***ROUTINES CALLED  DFLAGS,DIWINF,DSCLB,DSCLD,DWINF,DZERO
...
C
C
      DATA NEGONE,ZERO,ONE,TWO,THREE/-1.0D0,0.0D0,1.0D0,2.0D0,3.0D0/
C
C
...
C
      IF (SCLB(1).LE.ZERO) THEN
         CALL DSCLB(NP,BETA,EPSMAC,WORK(SSFI))
      ELSE
         CALL DCOPY(NP,SCLB,1,WORK(SSFI),1)
      END IF
      IF (SCLD(1,1).LE.ZERO) THEN
         LDTT = N
         CALL DSCLD(N,M,X,LDX,WORK(TTI),LDTT)
      ELSE
         IF (LDSCLD.EQ.1) THEN
            LDTT = 1
         ...
         ELSE
            LDTT = N
            DO 10 J=1,M
               CALL DCOPY(N,SCLD(1,J),1,WORK(TTI+(J-1)*LDTT),1)
   10       CONTINUE
         END IF
      END IF
C
