...
C***ROUTINES CALLED  SFLAGS,SINIWK,SIWINF,SJCK,SODCHK,
C                    SODMN,SODPER,SPACK,SMPREC,SWINF
...
C
            ...
            NTOL = -1
            NROW = -1
            CALL SJCK(FUN,JAC,N,NP,M,X,LDX,BETA,WORK(DELTAI),
     +                N,WORK(XPLUSI),N,
...
C
         IF (IWORK(MSGB).NE.1 .AND. IWORK(MSGX).NE.1) THEN
            CALL SPACK(NP,NPP,WORK(BETACI),BETA,IFIXB)
            IF (WORK(SSFI).GT.ZERO) THEN
               CALL SPACK(NP,NPP,WORK(SSI),WORK(SSFI),IFIXB)
            ELSE
               WORK(SSI) = WORK(SSFI)
            END IF
         END IF
