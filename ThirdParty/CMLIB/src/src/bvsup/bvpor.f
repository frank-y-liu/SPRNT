     ...
     1   NROWB,BETA,NFC,IFLAG,Z,MXNON,P,NTP,IP,W,NIV,YHP,U,V,COEF,S,
     2   STOWA,G,WORK,IWORK,NFCC)
...
C
C **********************************************************************
C***ROUTINES CALLED  BKSOL,LSSUDS,RKFAB,SCOEF,SDOT,STOR1,STWAY,SVECS
C***COMMON BLOCKS    ML15TO,ML18JR,ML8SZ
C***END PROLOGUE  BVPOR
C
      DIMENSION Y(NROWY,NXPTS),A(NROWA,NCOMP),ALPHA(NIC),B(NROWB,NCOMP),
     1          BETA(NFCC),P(NTP,MXNON),IP(NFCC,MXNON),
     2          U(NCOMP,NFC,NXPTS),V(NCOMP,NXPTS),W(NFCC,MXNON),
     3          COEF(NFCC),Z(MXNON),YHP(NCOMP,*),XPTS(NXPTS),S(*),
...
C
     ...
     1               NSWOT,KNSWOT,LOTJP,PWCND,TND,XOT,PX,XEND
      COMMON /ML18JR/NXPTSD,NICD,RE,AE,NOPG,MXNOND,NDISK,NTAPE,NEQ,
