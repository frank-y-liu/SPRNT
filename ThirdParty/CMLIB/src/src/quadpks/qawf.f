...
C***KEYWORDS  AUTOMATIC INTEGRATOR,CONVERGENCE ACCELERATION WITH DQEXT,
C             FOURIER,INTEGRATION BETWEEN ZEROES WITH DQAWOE,
...
10    IF(IER.EQ.6) LVL = 1
      IF(IER.NE.0) CALL XERROR( 'ABNORMAL RETURN FROM  QAWF',
     1  26,IER,LVL)
      RETURN
      END
