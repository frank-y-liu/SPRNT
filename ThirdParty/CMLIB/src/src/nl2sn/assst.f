      SUBROUTINE ASSST(D, IV, P, STEP, STLSTG, V, X, X0)
C
C  ***  ASSESS CANDIDATE STEP (***SOL VERSION 2.3)  ***
C
      INTEGER P, IV(32)
      REAL D(P), STEP(P), STLSTG(P), V(37), X(P), X0(P)
C
C  ***  PURPOSE  ***
C
...
C
      LOGICAL GOODX
      INTEGER I, NFC
      REAL EMAX, EMAXS, GTS, HALF, ONE, RELDX1, RFAC1, TWO,
     1                 XMAX, ZERO
C
...
C
 230  IV(XIRC) = IV(IRC)
 240  IF ( ABS(V(F)) .LT. V(AFCTOL)) IV(IRC) = 10
