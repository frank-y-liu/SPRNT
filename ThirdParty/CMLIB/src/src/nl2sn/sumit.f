...
C
      EXTERNAL ASSST, DBDOG, DEFLT, ITSUM, LITVMU, LIVMUL,
     1         LTVMUL, LUPDAT, LVMUL, PARCK, STOPX, VAXPY,
     2         VSCOPY, VVMULP, WZBFGS
      LOGICAL STOPX
C
...
C/7
C     PARAMETER (NEGONE=-1.E+0, ONE=1.E+0, ZERO=0.E+0)
C/
C
C+++++++++++++++++++++++++++++++  BODY  ++++++++++++++++++++++++++++++++
C
...
C
 70   CALL ITSUM(D, G, IV, LIV, LV, N, V, X)
 80   K = IV(NITER)
...
C
      CALL SCOPY(N, X,1,V(X01),1)
      CALL SCOPY(N, G,1,V(G01),1)
C
C  ***  CHECK STOPX AND FUNCTION EVALUATION LIMIT  ***
C
 100  IF (.NOT. STOPX(DUMMY)) GO TO 120
         IV(1) = 11
         GO TO 130
C
...
C
 140  STEP1 = IV(STEP)
      ...
      IF (IV(KAGQT) .GE. 0) GO TO 150
         ...
         V(DST0) = SNRM2(N, V(STEP1),1)
         CALL VVMULP(N, V(DG1), V(DG1), D, -1)
         CALL LTVMUL(N, V(STEP1), V(L), V(DG1))
         V(GTHG) = SNRM2(N, V(STEP1),1)
         IV(KAGQT) = 0
 150  CALL DBDOG(V(DG1), G, LV, N, V(NWTST1), V(STEP1), V)
...
C
 160  STEP1 = IV(STEP)
      LSTGST = IV(STLSTG)
      X01 = IV(X0)
      CALL ASSST(D, IV, N, V(STEP1), V(LSTGST), V, X, V(X01))
C
      K = IV(IRC)
...
C        ***  DO GRADIENT TESTS  ***
C
         IF (SNRM2(N, V(TEMP1),1) .LE. V(DGNORM) * V(TUNER4))
     1                  GO TO 230
              IF (SDOT(N, G,1,V(STEP1),1)
...
C
 260  IV(1) = IV(CNVCOD)
      IV(CNVCOD) = 0
 270  CALL ITSUM(D, G, IV, LIV, LV, N, V, X)
C
 999  RETURN
C
C  ***  LAST CARD OF SUMIT FOLLOWS  ***
      END
