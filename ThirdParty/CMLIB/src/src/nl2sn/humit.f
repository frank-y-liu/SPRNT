...
C
      EXTERNAL ASSST, DEFLT, DUPDU, GQTST, ITSUM, PARCK,
     1         SLVMUL, STOPX, VAXPY, VSCOPY
      LOGICAL STOPX
C
...
C
      ...
      DO 140 I = 1, N
         ...
         K = K + 1
 140     CONTINUE
      V(RADIUS) = V(RADFAC) * SNRM2(N, V(STEP1),1)
C
...
C     ***  DO GRADIENT TESTS  ***
C
      IF (SNRM2(N, V(TEMP1),1) .LE. V(DGNORM) * V(TUNER4)) GO TO 300
...
C
 320  IV(1) = IV(CNVCOD)
      IV(CNVCOD) = 0
 330  CALL ITSUM(D, G, IV, LIV, LV, N, V, X)
C
 999  RETURN
C
