...
C
      INTEGER KA, P
      REAL D(P), DIG(P), DIHDI(*), L(*), V(21), STEP(P),
     1                 W(*)
...
C
      EXTERNAL LITVMU, LIVMUL, LSQRT, LSVMIN
      REAL LSVMIN
C
C  ***  SUBSCRIPTS FOR V  ***
C
...
C     ***  IF NOT YET AVAILABLE, OBTAIN MACHINE DEPENDENT VALUE DGXFAC.
 370  IF (DGXFAC .EQ. ZERO) DGXFAC = EPSFAC * R1MACH(4)
C
...
C
      T1 = ZERO
      T = SI*(ALPHAK*SW - HALF*SI*(ALPHAK + T*SDOT(P,W(X),1,W,1)))
