...
C***ROUTINES CALLED  LA05ES,XERROR,XERRWV,XSETUN
C***COMMON BLOCKS    LA05DS
C***REVISION HISTORY  (YYMMDD)
C   000330  Modified array declarations.  (JEC)
C           Changed upper limit on DO from LAST to LAST-1.
...
C     THE FOLLOWING INSTRUCTIONS IMPLEMENT THE FAILURE EXITS.
  590 IF (LP.LE.0) GO TO 600
      NERR = -6
      IOPT = 1
      NI = 1
      NR = 0
      CALL XERRWV(
     *'LA05CS. SING. MATRIX AFTER REPLACEMENT OF COL.  HERE I1=INDEX.',
     * 62, NERR, IOPT, NI, MM, IDUMMY, NR, DUMMY, DUMMY)
  600 CONTINUE
      G = -6.
      GO TO 630
  610 CONTINUE
     ...
     * 'LA05CS. LENGTHS OF ARRAYS A(*) AND IND(*,2) ARE TOO SMALL.',
     * 58, NERR, IOPT)
      G = -7.
      GO TO 630
  620 CONTINUE
      ...
      IOPT = 1
      IF (LP.GT.0) CALL XERROR(
     * 'LA05CS. EARLIER ENTRY GAVE ERROR RETURN.', 40, NERR, IOPT)
  630 RETURN
      END
