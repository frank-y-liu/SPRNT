...
C
C***ROUTINES CALLED  LA05ES,MC20AS,XERROR,XERRWV,XSETUN
C***COMMON BLOCKS    LA05DS
C***END PROLOGUE  LA05AS
      INTEGER IP(N,2)
      INTEGER IND(IA,2), IW(N,8)
      REAL A(IA), AMAX, AU, AM, G, U, SMALL, W(N)
...
C***FIRST EXECUTABLE STATEMENT  LA05AS
      DATA EPS /0./
      IF (EPS.NE.0.) GO TO 30
      EPS=1.
   10 CONTINUE
      IF (EPS+1..EQ.1.) GO TO 20
      EPS = EPS*0.5
      GO TO 10
   20 EPS = EPS + EPS
   30 CONTINUE
...
C     THE FOLLOWING INSTRUCTIONS IMPLEMENT THE FAILURE EXITS.
  660 CONTINUE
      ...
      NI = 2
      NR = 0
      IF (LP.GT.0) CALL XERRWV(
     *   'LA05AS. MORE THAN ONE MATRIX ENTRY.  HERE I1=ROW, I2=COL.',
     * 57, NERR, IOPT, NI, IR, J, NR, DUMMY, DUMMY)
      G = -4.
      GO TO 760
  670 CONTINUE
      NERR = -1
      IOPT = 1
      IF (LP.GT.0) CALL XERROR(
     *   'LA05AS. THE ORDER OF THE SYSTEM, N, IS NOT POSITIVE.', 52,
     * NERR, IOPT)
      G = -1.
      GO TO 760
  680 CONTINUE
      ...
      NI = 1
      NR = 0
      IF (LP.LE.0) GO TO 690
      CALL XERRWV('LA05AS. ELEMENT K IS OUT OF BOUNDS.  HERE I1=K.',
     * 47, NERR, IOPT, NI, K, IDUMMY, NR, DUMMY, DUMMY)
      NI = 2
      CALL XERRWV('LA05AS.  HERE I1=ROW, I2=COL.', 29, NERR, IOPT, NI,
     * I, J, NR, DUMMY, DUMMY)
  690 CONTINUE
      G = -3.
      GO TO 760
  700 CONTINUE
      ...
      NR = 0
      IF (LP.GT.0) CALL XERRWV(
     * 'LA05AS. ROW OR COL HAS NO ELEMENTS.  HERE I1=INDEX.', 51,
     * NERR, IOPT, NI, L, IDUMMY, NR, DUMMY, DUMMY)
      G = -2.
      GO TO 760
  710 CONTINUE
      NERR = -7
      IOPT = 1
      IF (LP.GT.0) CALL XERROR(
     * 'LA05AS. LENGTHS OF ARRAYS A(*) AND IND(*,2) ARE TOO SMALL.',
     * 58, NERR, IOPT)
      G = -7.
      GO TO 760
  720 IPV = IPV + 1
  ...
  730 CONTINUE
      IF (LP.LE.0) GO TO 750
      NERR = -5
      IOPT = 1
      NI = 1
      NR = 0
      CALL XERRWV(
     * 'LA05AS. DEPENDENT ROWS OR COLS.  HERE I1=1 (ROWS), =2 (COLS)',
     * 60, NERR, IOPT, NI, L, IDUMMY, NR, DUMMY, DUMMY)
      I = 1
      NI = 2
  740 CONTINUE
      IF (I+1.GT.IPV) NI = 1
      CALL XERRWV('LA05AS. DEPENDENT VECTOR INDICES ARE I1 AND I2.',
     * 47, NERR, IOPT, NI, IW(I,1), IW(I+1,1), NR, DUMMY, DUMMY)
      I = I + 2
