...
c
      ...
      integer           p,q,r,s
c
      real              acu,alpha1,alpha2,beta1,beta2,big,d,delhat
      real              delstr,devsgn,div,dsubi,dsubr,dsubs,lambda
      real              ratio,r1,r2,save,signp,signq,signr
      real              signs,s1,s2,sumr,sums,top
      real              x(*), y(*)
c
c---------------------
...
c
      p=1
      do 110 i=2,n
          if(abs(x(p)-x(i)).gt.acu)goto 120
  110 continue
c
      ifault=2
      p = 0
...
c
      r1=d*(x(q)-x(r))
      r2=d*(x(r)-x(p))
      signp=sign(1.,-signr*r1)
      signq=sign(1.,-signr*r2)
      sumr=signr-signp*r1-signq*r2
      lambda=abs(dsubr/sumr)
...
c              violates the current solution
c
  140 s=0
      dsubs=acu
      do 150 i=1,n
          dsubi=abs(beta1+beta2*x(i)-y(i))-lambda
...
c
c     step 4 - determine which constraint will not be binding
c              when the s-th constraint is made feasible (by
c              locating the minimum ratio)
c
  160 ratio=big
c
c     if the signs are not the same, the p-th constraint is not a
c     candidate
c
      if(signs*sign(1.,s1).ne.signp)goto 170
c
c     ignore ratio if denominator is effectively zero
c
      if(abs(s1).lt.acu)goto 170
      ratio=abs(r1/s1)
c
c     if the signs are not the same, the q-th constraint is not a
c     candidate
c
  170 if(signs*sign(1.,s2).ne.signq)goto 180
c
c     ignore ratio if denominator is effectively zero
c
      if(abs(s2).lt.acu)goto 180
c
c     if minimum is p-th, go to process p-th
c
      if(abs(r2/s2).lt.ratio)goto 200
      go to 210
c
c     if minimum is q-th, go to switch the p-th and q-th constraints so
c     that the algorithm will always process the p-th constraint
c
  180 if(ratio.lt.big)goto 210
c
c     step 5 - no minimum was located, so process the r-th constraint
c
      delhat=abs(dsubs/sums)
c
...
c
      ...
      signr=signs
      signs=-save
      dsubs=abs(sums*delhat) -2.*lambda
...
c              switch p-th and q-th constraints
c
  200 isave=p
      p=q
      q=isave
...
c
  210 delhat=abs(r1*dsubs/(r1*sums+s1*sumr))
      top=-2.*lambda*r1
      div=r1 +r1 +signp*sumr
      if(sign(1.,top).ne.sign(1.,div)) go to 220
      if(abs(div).lt.acu) go to 220
      delstr=top/div
c
...
c
      ...
      dsubs=dsubs-delstr*abs(sums+s1*sumr/r1)
...
c
  220 signp=signs
      ...
      kount=kount+1
c
c     update lambda, compute the new solution - goto step 3 to
