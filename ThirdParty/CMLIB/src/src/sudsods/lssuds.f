      SUBROUTINE LSSUDS(A,X,B,N,M,NRDA,U,NRDU,IFLAG,MLSO,IRANK,ISCALE,
     1   Q,DIAG,KPIVOT,S,DIV,TD,ISFLG,SCALES)
C***BEGIN PROLOGUE  LSSUDS
C***REFER TO  SUDS
C
C    LSSUDS solves the underdetermined system of equations  A Z = B,
C    where A is N by M and N .le. M. In particular,if rank A equals IRA,
C    a vector X and a matrix U are determined such that X is the UNIQUE
C    solution of smallest length, satisfying A X = B, and the columns of
C    U form an orthonormal basis for the null space of A, satisfying
C    A U = 0 . Then all solutions Z are given by
...
C      DIV,TD,SCALES    storage (execpt for SCALES which is M)
...
C
C *********************************************************************
C***ROUTINES CALLED  J4SAVE,OHTROL,ORTHOR,R1MACH,SDOT,XERMAX,XERROR,
C                    XGETF,XSETF
C***END PROLOGUE  LSSUDS
      DIMENSION A(NRDA,M),X(M),B(N),U(NRDU,1),Q(NRDA,M),
     1          DIAG(N),KPIVOT(N),S(N),DIV(N),TD(N),SCALES(M)
C
C **********************************************************************
C
...
C     INVALID INPUT FOR LSSUDS
    1 IFLAG=2
      CALL XERROR ( 'LSSUDS-INVALID IMPUT PARAMETERS.',32,2,1)
      RETURN
C
    5 CALL XGETF(NFATAL)
...
C
   ...
   55       S(J)=S(J)+GAM*Q(J,K)
      ...
      IF (RES .LE. SS*(10.*AMAX1(10.**ISFLG,10.*URO))**2) GO TO 60
C
