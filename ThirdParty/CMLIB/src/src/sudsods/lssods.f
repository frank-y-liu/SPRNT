      SUBROUTINE LSSODS(A,X,B,M,N,NRDA,IFLAG,IRANK,ISCALE,Q,DIAG,
     1   KPIVOT,ITER,RESNRM,XNORM,Z,R,DIV,TD,SCALES)
...
C     original matrix A.
C
C    Modeled after the ALGOL codes from the following
C    References
...
C
C **********************************************************************
C***ROUTINES CALLED  J4SAVE,OHTROR,ORTHOL,R1MACH,SDOT,SDSDOT,XERMAX,
C                    XERROR,XGETF,XSETF
C***END PROLOGUE  LSSODS
      DIMENSION A(NRDA,N),X(N),B(M),Q(NRDA,N),DIAG(N),
     1          Z(N),KPIVOT(N),R(M),DIV(N),TD(N),SCALES(N)
C
C **********************************************************************
C
...
C     INVALID INPUT FOR LSSODS
    1 IFLAG=2
      CALL XERROR ( 'LSSODS-INVALID INPUT PARAMETERS.',32,2,1)
      RETURN
C
    5 CALL XGETF (NFATAL)
...
C
   ...
   10       Q(K,J)=A(K,J)
C
C     USE ORTHOGNAL TRANSFORMATIONS TO REDUCE Q TO
...
C
C     FOR RANK DEFICIENT PROBLEMS USE ADDITIONAL ORTHOGNAL
...
C
         IFLAG=4
         CALL XERROR (  'LSSODS-PROBLEM MAY BE ILL-CONDITIONED.  MAXIMAL
     1 MACHINE ACCURACY IS NOT ACHIEVABLE.',83,3,1)
         RETURN
C
   90    IFLAG=5
         CALL XERROR ( 'LSSODS-PROBLEM IS VERY ILL-CONDITIONED.  ITERATI
     1VE IMPROVEMENT IS INEFFECTIVE.',78,8,1)
         RETURN
C
  100    ZNRM0=ZNORM
C **********************************************************************
...
C **********************************************************************
      IFLAG=6
         CALL XERROR ( 'LSSODS-CONVERGENCE HAS NOT BEEN OBTAINED WITH AL
     1LOWABLE NUMBER OF ITERATIVE IMPROVEMENT STEPS.',94,8,1)
C
      RETURN
      END
