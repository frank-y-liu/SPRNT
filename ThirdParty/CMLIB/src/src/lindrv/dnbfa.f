...
C
C      IDAMAX,DAXPY,DSCAL,DSWAP
C     Fortran  MIN0
C***REFERENCES  SUBROUTINE DNBFA WAS DEVELOPED BY GROUP C-3, LOS ALAMOS
...
C
        DO 10 J=1,MP
          CALL DAXPY(LM,ABE(K,ML1+J),ABE(LM+K,LM2),LDB,ABE(LM+K,LM2+J),L
     1DB)
   10   CONTINUE
        GO TO 30
   20   CONTINUE
