...
C
C     SLATEC DNBFA
C      DDOT,DAXPY,DSCAL,DASUM
C     Fortran  DABS,DMAX1,MAX0,MIN0,DSIGN
C***REFERENCES  SUBROUTINE DNBCO WAS DEVELOPED BY GROUP C-3, LOS ALAMOS
...
C***FIRST EXECUTABLE STATEMENT  DNBCO
      ...
      DO 10 J = 1, N
        ...
        NL = MIN0(ML,N-J)
        L = 1 + NU + NL
        ANORM = DMAX1(ANORM,DASUM(L,ABE(J+NL,ML1-NL),LDB))
   10 CONTINUE
C
C     FACTOR
C
...
C
   ...
   20 CONTINUE
      ...
      DO 100 K = 1, N
        ...
        IF (DABS(EK-Z(K)) .LE. DABS(ABE(K,ML1))) GO TO 30
          S = DABS(ABE(K,ML1))/DABS(EK-Z(K))
          CALL DSCAL(N,S,Z,1)
          EK = S*EK
   30   CONTINUE
        WK = EK - Z(K)
...
C
      DO 160 KB = 1, N
        K = N + 1 - KB
        IF (DABS(Z(K)) .LE. DABS(ABE(K,ML1))) GO TO 150
          S = DABS(ABE(K,ML1))/DABS(Z(K))
          CALL DSCAL(N,S,Z,1)
          YNORM = S*YNORM
  150   CONTINUE
