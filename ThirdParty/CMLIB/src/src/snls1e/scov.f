...
C
C     CALCULATE SIGMA =( SUM OF THE SQUARED RESIDUALS) / FLOAT(M-N)
      IFLAG=1
...
300   CONTINUE
      ...
      IF (SING) INFO=2
      IF (INFO .LT. 0) CALL XERROR( 'SCOV   -- EXECUTION TERMINATED BECA
     1USE USER SET IFLAG NEGATIVE.',63,1,1)
      IF (INFO .EQ. 0) CALL XERROR( 'SCOV   -- INVALID INPUT PARAMETER.'
     1,34,2,1)
      IF (INFO .EQ. 2) CALL XERROR( 'SCOV   -- SINGULAR JACOBIAN MATRIX,
     1COVARIANCE MATRIX CANNOT BE CALCULATED.',74,1,1)
      RETURN
      END
