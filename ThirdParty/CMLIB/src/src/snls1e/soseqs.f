      SUBROUTINE SOSEQS(FNC,N,S,RTOLX,ATOLX,TOLF,IFLAG,MXIT,NCJS,NSRRC,
     1   NSRI,IPRINT,FMAX,C,NC,B,P,TEMP,X,Y,FAC,IS)
...
C
C
      DIMENSION S(N), C(NC), B(N), IS(N), P(N), TEMP(N), X(N), Y(N),
     1          FAC(N)
C
...
C
        ...
        DO 180 J=1,N
          YJ = Y(J)
          YNORM = AMAX1(YNORM,ABS(YJ))
          JS = IS(J)
          X(JS) = TEMP(JS) + YJ
          XNORM = AMAX1(XNORM,ABS(X(JS)))
  180   CONTINUE
C
C
