...
C***PURPOSE  Computes the d.p. arc hyperbolic Tangent.
C***DESCRIPTION
C
...
C
 10   Y = DABS(X)
      IF (Y.GE.1.D0) CALL XERROR ( 'DATANH  DABS(X) GE 1', 20, 2, 2)
C
      IF (1.D0-Y.LT.DXREL) CALL XERROR ( 'DATANH  ANSWER LT HALF PRECISI
     1ON BECAUSE DABS(X) TOO NEAR 1',   59, 1, 1)
C
      DATANH = X
      IF (Y.GT.SQEPS .AND. Y.LE.0.5D0) DATANH = X*(1.0D0 +
     1  DCSEVL (8.D0*X*X-1.D0, ATNHCS, NTERMS) )
      IF (Y.GT.0.5D0) DATANH = 0.5D0*DLOG ((1.0D0+X)/(1.0D0-X) )
C
      RETURN
      END
