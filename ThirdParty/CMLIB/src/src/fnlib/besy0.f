...
C
      ...
      XMAX = 1.0/R1MACH(4)
C
 10   IF (X.LE.0.) CALL XERROR ( 'BESY0   X IS ZERO OR NEGATIVE', 29,
     1  1, 2)
      IF (X.GT.4.0) GO TO 20
C
      Y = 0.
      IF (X.GT.XSML) Y = X*X
      BESY0 = TWODPI*ALOG(0.5*X)*BESJ0(X) + .375 + CSEVL (.125*Y-1.,
     1  BY0CS, NTY0)
      RETURN
C
 20   IF (X.GT.XMAX) CALL XERROR ( 'BESY0   NO PRECISION BECAUSE X IS BI
     1G', 37, 2, 2)
C
      Z = 32.0/X**2 - 1.0
