...
C***CATEGORY NO.  C10A1
C***KEYWORDS  BESSEL FUNCTION,ORDER ONE,SECOND KIND,SPECIAL FUNCTION
...
C***REFERENCES  (NONE)
C***ROUTINES CALLED  BESJ1,CSEVL,INITS,R1MACH,XERROR
...
C
      ...
      XMAX = 1.0/R1MACH(4)
C
 10   IF (X.LE.0.) CALL XERROR ( 'BESY1   X IS ZERO OR NEGATIVE', 29,
     1  1, 2)
      IF (X.GT.4.0) GO TO 20
C
      IF (X.LT.XMIN) CALL XERROR ( 'BESY1   X SO SMALL Y1 OVERFLOWS',
     1  31, 3, 2)
      Y = 0.
      IF (X.GT.XSML) Y = X*X
     ...
     1  (0.5 + CSEVL (.125*Y-1., BY1CS, NTY1))/X
      RETURN
C
 20   IF (X.GT.XMAX) CALL XERROR ( 'BESY1   NO PRECISION BECAUSE X IS BI
     1G', 37, 2, 2)
C
      Z = 32.0/X**2 - 1.0
