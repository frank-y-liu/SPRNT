...
C***PURPOSE  Evaluate DLOG(1+X) from second order relative accuracy so
...
C
     ...
     1  LN22CS, NTLN22)
      RETURN
C
 20   IF (X.LT.XMIN) CALL XERROR ( 'D9LN2R  ANSWER LT HALF PRECISION BEC
     1AUSE X IS TOO NEAR -1',     57, 1, 1)
      IF (X.GT.XMAX) CALL XERROR ( 'D9LN2R  NO PRECISION IN ANSWER BECAU
     1SE X IS TOO BIG', 51,       3, 2)
      IF (X.GT.XBIG) CALL XERROR ( 'D9LN2R  ANSWER LT HALF PRECISION BEC
