...
C***CATEGORY NO.  C1
C***KEYWORDS  DOUBLE PRECISION,FACTORIAL,SPECIAL FUNCTION
C***AUTHOR  FULLERTON, W., (LANL)
C***PURPOSE  Computes the d.p. factorial of N.
C***DESCRIPTION
C
C DFAC(N) calculates the double precision factorial for integer
C argument N.
C***REFERENCES  (NONE)
C***ROUTINES CALLED  D9LGMC,DGAMLM,XERROR
C***END PROLOGUE  DFAC
...
C***FIRST EXECUTABLE STATEMENT  DFAC
      ...
      NMAX = XMAX - 1.D0
C
 10   IF (N.LT.0) CALL XERROR ( 'DFAC    FACTORIAL OF NEGATIVE INTEGER U
     1NDEFINED', 47, 1, 2)
C
      IF (N.LE.30) DFAC = FACN(N+1)
      IF (N.LE.30) RETURN
C
      IF (N.GT.NMAX) CALL XERROR ( 'DFAC    N SO BIG FACTORIAL(N) OVERFL
     1OWS', 39, 2, 2)
C
      X = N + 1
