...
C***PURPOSE  Computes the logarithmic confluent hypergeometric function.
C***DESCRIPTION
C
...
C***REFERENCES  (NONE)
C***ROUTINES CALLED  EXPREL,GAMMA,GAMR,POCH,POCH1,R1MACH,R9CHU,XERROR
C***END PROLOGUE  CHU
      EXTERNAL GAMMA
...
C
      IF (X.EQ.0.0) CALL XERROR ( 'CHU     X IS ZERO SO CHU IS INFINITE'
     1  , 36, 1, 2)
      IF (X.LT.0.0) CALL XERROR ( 'CHU     X IS NEGATIVE, USE CCHU',
     1  31, 2, 2)
C
      IF (AMAX1(ABS(A),1.0)*AMAX1(ABS(1.0+A-B),1.0).LT.0.99*ABS(X))
     1  GO TO 120
C
...
C
      IF (ABS(1.0+A-B).LT.SQRT(EPS)) CALL XERROR ( 'CHU     ALGORITHM IS
     1 BAD WHEN 1+A-B IS NEAR ZERO FOR SMALL X',  60, 10, 2)
C
      AINTB = AINT(B+0.5)
...
C
 ...
 80   CONTINUE
      CALL XERROR ( 'CHU     NO CONVERGENCE IN 1000 TERMS OF THE ASCENDI
     1NG SERIES',  60, 3, 2)
C
