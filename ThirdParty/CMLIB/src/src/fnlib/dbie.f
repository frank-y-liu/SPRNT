...
C***CATEGORY NO.  C10D
C***KEYWORDS  BAIRY FUNCTION,DOUBLE PRECISION,EXPONENTIALLY SCALED,
C             SPECIAL FUNCTION
C***AUTHOR  FULLERTON, W., (LANL)
C***PURPOSE  Calculates the d.p. Bairy function for negative X and an
C            exponentially scaled Bairy function for positive X.
C***DESCRIPTION
C
...
C***END PROLOGUE  DBIE
     ...
     2  X32SML, Z, D1MACH, DCSEVL
      ...
      DATA X3SML, X32SML, XBIG / 3*0.0D0 /
...
C
 20   IF (X.GT.1.0D0) GO TO 30
     ...
     1  DCSEVL (Z, BIGCS, NBIG) )
      IF (X.GT.X32SML) DBIE = DBIE * DEXP(-2.0D0*X*DSQRT(X)/3.0D0)
      RETURN
C
 30   IF (X.GT.2.0D0) GO TO 40
      Z = (2.0D0*X**3 - 9.0D0)/7.0D0
      DBIE = DEXP(-2.0D0*X*DSQRT(X)/3.0D0) * (1.125D0 +
...
C
 40   IF (X.GT.4.0D0) GO TO 50
      SQRTX = DSQRT(X)
      Z = ATR/(X*SQRTX) + BTR
      DBIE = (0.625D0 + DCSEVL (Z, BIP1CS, NBIP1))/DSQRT(SQRTX)
      RETURN
C
 50   SQRTX = DSQRT(X)
      Z = -1.0D0
      IF (X.LT.XBIG) Z = 16.D0/(X*SQRTX) - 1.0D0
      DBIE = (0.625D0 + DCSEVL (Z, BIP2CS, NBIP2))/DSQRT(SQRTX)
      RETURN
C
      END
