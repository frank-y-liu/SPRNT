...
C***CATEGORY NO.  C8C
C***KEYWORDS  DAWSON,DAWSON-S,SPECIAL FUNCTION
C***AUTHOR  FULLERTON, W., (LANL)
C***PURPOSE  Computes Dawson's function.
C***DESCRIPTION
C
...
C
      XSML = SQRT (1.5*EPS)
      XBIG = SQRT (0.5/EPS)
      XMAX = EXP (AMIN1 (-ALOG(2.*R1MACH(1)), ALOG(R1MACH(2))) - 1.0)
C
 10   Y = ABS(X)
...
C
      ...
      RETURN
C
 40   CALL XERROR ( 'DAWS    ABS(X) SO LARGE DAWS UNDERFLOWS', 39, 1, 1)
      DAWS = 0.0
      RETURN
C
      END
