...
C
      ...
      NERR=1
      IOPT=2
      CALL XERROR( 'HFTI MDA.LT.M.. PROBABLE ERROR.',31,NERR,IOPT)
      RETURN
    5 CONTINUE
C
      IF (.NOT.(NB.GT.1.AND.MAX0(M,N).GT.MDB)) GO TO 6
      NERR=2
      IOPT=2
      CALL XERROR( 'HFTI MDB.LT.MAX(M,N).AND.NB.GT.1. PROBABLE ERROR.',
     1 49,NERR,IOPT)
      RETURN
    6 CONTINUE
C
