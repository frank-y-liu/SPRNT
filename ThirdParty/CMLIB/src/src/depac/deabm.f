      SUBROUTINE DEABM(F,NEQ,T,Y,TOUT,INFO,RTOL,ATOL,IDID,RWORK,LRW,
...
C
C   DEABM uses subprograms DE2, STEP2, INTRP, HSTART, VNORM, R1MACH, and
...
C             You must have  LRW .GE. 120+21*NEQ
C
...
C***FIRST EXECUTABLE STATEMENT  DEABM
      ...
      CALL XERRWV( 'DEABM -- AN APPARENT INFINITE LOOP HAS BEEN DETECTED
...
C
    5 IDID=0
      IF (LRW .GE. 120+21*NEQ) GO TO 10
      CALL XERRWV( 'DEABM -- LENGTH OF RWORK ARRAY MUST BE AT LEAST  120
     1 + 21*NEQ.  YOU            HAVE CALLED THE CODE WITH  LRW = (I1).'
     2,117,1,1,1,LRW,0,0,0.,0.)
      IDID=-33
C
   10 IF (LIW .GE. 33) GO TO 25
      CALL XERRWV( 'DEABM -- LENGTH OF IWORK ARRAY MUST BE AT LEAST  33.
...
C
 50   CALL DE2(F,NEQ,T,Y,TOUT,INFO,RTOL,ATOL,IDID,RWORK(IYPOUT),
     1         RWORK(IYP),RWORK(IYY),RWORK(IWT),RWORK(IP),RWORK(IPHI),
     2         RWORK(IALPHA),RWORK(IBETA),RWORK(IPSI),RWORK(IV),
     3         RWORK(IW),RWORK(ISIG),RWORK(IG),RWORK(11),RWORK(12),
     4         RWORK(13),RWORK(IHOLD),RWORK(ITOLD),RWORK(IDELSN),
     5         RWORK(1),RWORK(ITWOU),RWORK(IFOURU),START,PHASE1,NORND,
     6         STIFF,INTOUT,IWORK(26),IWORK(27),IWORK(28),IWORK(29),
     7         IWORK(30),IWORK(31),IWORK(32),RPAR,IPAR)
C
      IWORK(21) = -1
      IF (START) IWORK(21) = 1
      IWORK(22) = -1
