...
C***FIRST EXECUTABLE STATEMENT  SGEEV
      IF(N .GT. LDA) CALL XERROR( 'SGEEV-N .GT. LDA.',17,1,1)
      IF (N .GT. LDA) RETURN
      IF(N .LT. 1) CALL XERROR( 'SGEEV-N .LT. 1',14,2,1)
      IF(N .LT. 1) RETURN
      IF(N .EQ. 1 .AND. JOB .EQ. 0) GO TO 35
      MDIM = LDA
      IF(JOB .EQ. 0) GO TO 5
      IF(N .GT. LDV) CALL XERROR( 'SGEEV-JOB .NE. 0, AND N .GT. LDV.',
     1  33,3,1)
      IF(N .GT. LDV) RETURN
      IF(N .EQ. 1) GO TO 35
C
...
C
      MDIM = MIN0(LDA,LDV)
      IF(LDA.LT.LDV) CALL XERROR( 'SGEEV-LDA.LT.LDV,  ELEMENTS OF V OTHE
     1R THAN THE N BY N OUTPUT ELEMENTS HAVE BEEN CHANGED.',89,5,0)
      IF(LDA.LE.LDV) GO TO 5
      CALL XERROR(  'SGEEV-LDA.GT.LDV, ELEMENTS OF A OTHER THAN THE N BY
