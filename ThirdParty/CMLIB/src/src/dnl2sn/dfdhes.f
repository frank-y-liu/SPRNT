      SUBROUTINE DFDHES(D, G, IRT, IV, LIV, LV, P, V, X)
C
C  ***  COMPUTE FINITE-DIFFERENCE HESSIAN, STORE IT IN V STARTING
C  ***  AT V(IV(FDH)) = V(-IV(H)).
C
C  ***  IF IV(COVREQ) .GE. 0 THEN DFDHES USES GRADIENT DIFFERENCES,
C  ***  OTHERWISE FUNCTION DIFFERENCES.  STORAGE IN V IS AS IN DGLRIT.
C
C IRT VALUES...
...
C
C/6
      DATA COVMAT/26/, COVREQ/15/, DELTA/52/, DELTA0/44/, DLTFDC/42/,
     1     F/10/, FDH/74/, FX/53/, H/56/, KAGQT/33/, MODE/35/,
     2     NFGCAL/7/, SAVEI/63/, SWITCH/12/, TOOBIG/2/, W/65/,
     3     XMSAVE/51/
C/7
C     PARAMETER (COVMAT=26, COVREQ=15, DELTA=52, DELTA0=44, DLTFDC=42,
C    1           F=10, FDH=74, FX=53, H=56, KAGQT=33, MODE=35, NFGCAL=7,
...
C
      GSAVE1 = IV(W) + P
      IF (M .GT. 0) GO TO 20
C        ***  FIRST CALL ON DFDHES.  SET GSAVE = G, TAKE FIRST STEP  ***
         CALL DCOPY(P, G,1,V(GSAVE1),1)
         IV(SWITCH) = IV(NFGCAL)
         GO TO 90
C
 20   DEL = V(DELTA)
...
C
 ...
 220  V(F) = V(FX)
      ...
      IF (KIND .LT. 0) GO TO 999
         IV(NFGCAL) = IV(SWITCH)
         GSAVE1 = IV(W) + P
         CALL DCOPY(P, V(GSAVE1),1,G,1)
         GO TO 999
C
 999  RETURN
C  ***  LAST CARD OF DFDHES FOLLOWS  ***
      END
