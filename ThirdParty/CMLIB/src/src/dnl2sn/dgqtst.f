      SUBROUTINE DGQTST(D, DIG, DIHDI, KA, L, P, STEP, V, W)
C
...
C
      EXTERNAL DLITVM, DLIVMU, DLSQRT, DLSVMN
...
C  ***  TRY ALPHA = 0  ***
C
 30   CALL DLSQRT(1, P, L, DIHDI, IRC)
...
C     ***  POSITIVE DEFINITE H -- COMPUTE UNMODIFIED NEWTON STEP.  ***
 50   LK = ZERO
      T = DLSVMN(P, L, W(Q), W(Q))
      IF (T .GE. ONE) GO TO 60
         IF (BIG .LE. ZERO) BIG = D1MACH(2)
         IF (V(DGNORM) .GE. T*T*BIG) GO TO 70
 60   CALL DLIVMU(P, W(Q), L, DIG)
      GTSTA = DDOT(P, W(Q),1,W(Q),1)
      V(NREDUC) = HALF * GTSTA
      CALL DLITVM(P, W(Q), L, W(Q))
      DST = DNRM2(P, W(Q),1)
      V(DST0) = DST
      PHI = DST - RAD
...
C  ***  FROM DLSVMN TO OBTAIN APPROXIMATE EIGENVECTOR CORRESPONDING
C  ***  TO SMALLEST EIGENVALUE OF (D**-1)*H*(D**-1).  DLSVMN RETURNS
C  ***  X AND W WITH  L*W = X.
C
 340  T = DLSVMN(P, L, W(X), W)
C
C     ***  NORMALIZE W  ***
      DO 350 I = 1, P
 350     W(I) = T*W(I)
...
C     ***  IF NOT YET AVAILABLE, OBTAIN MACHINE DEPENDENT VALUE DGXFAC.
 370  IF (DGXFAC .EQ. ZERO) DGXFAC = EPSFAC * D1MACH(4)
C
...
C
      T1 = ZERO
      T = SI*(ALPHAK*SW - HALF*SI*(ALPHAK + T*DDOT(P,W(X),1,W,1)))
