      SUBROUTINE DQRADR(P, QTR, RMAT, W, Y)
C
...
C
      INTEGER P
      DOUBLE PRECISION QTR(P), RMAT(*), W(P), Y
C     DIMENSION RMAT(P*(P+1)/2)
C
C  ***  LOCAL VARIABLES  ***
C
      INTEGER I, J, II, IJ, IP1
      DOUBLE PRECISION ONE, DABS, T, V, ZERO, U1
      DOUBLE PRECISION U2, RI, WI, RW, WR, DSQRT
      DATA ONE/1.D+0/
      DATA ZERO/0.D+0/
C
C------------------------------ BODY -----------------------------------
...
C           *** SWAP W AND ROW I OF RMAT ***
 ...
 20      IP1 = I+1
         IJ = II+I
         IF (DABS(WI) .LE. DABS(RI)) GO TO 40
            RW = RI/WI
            T = DSQRT(ONE+RW**2)
            IF (RW .GT. ZERO) T = -T
            V = RW-T
