      DOUBLE PRECISION FUNCTION DLSVMN(P, L, X, Y)
C
...
C
      ...
      PM1 = P - 1
C
C  ***  FIRST CHECK WHETHER TO RETURN DLSVMN = 0 AND INITIALIZE X  ***
C
      II = 0
      J0 = P*PM1/2
      JJ = J0 + P
...
C       ***  UPDATE PARTIAL SUMS  ***
         IF (JM1 .GT. 0) CALL DVAXPY(JM1, X, XPLUS, L(J0+1), X)
 50      CONTINUE
C
C  ***  NORMALIZE X  ***
C
 60   T = ONE/DNRM2(P, X,1)
      DO 70 I = 1, P
 70      X(I) = T*X(I)
C
C  ***  SOLVE L*Y = X AND RETURN DLSVMN = 1/TWONORM(Y)  ***
C
      DO 100 J = 1, P
         JM1 = J - 1
         J0 = J*JM1/2
         JJ = J0 + J
      T=ZERO
      IF(JM1.GT.0)T = DDOT(JM1,L(J0+1),1,Y,1)
      Y(J)=(X(J)-T)/L(JJ)
 100     CONTINUE
C
      DLSVMN = ONE/DNRM2(P, Y,1)
      GO TO 999
C
 110  DLSVMN = ZERO
 999  RETURN
C  ***  LAST CARD OF DLSVMN FOLLOWS  ***
      END
