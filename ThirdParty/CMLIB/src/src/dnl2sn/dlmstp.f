      SUBROUTINE DLMSTP(D, G, IERR, IPIVOT, KA, P, QTR, R, STEP, V, W)
C
...
C             DLMSTP FOR THE CURRENT R AND QTR.  ON OUTPUT KA CON-
...
C
C DLITVM - APPLY INVERSE-TRANSPOSE OF COMPACT LOWER TRIANG. MATRIX.
C DLIVMU - APPLY INVERSE OF COMPACT LOWER TRIANG. MATRIX.
C
C  ***  REFERENCES  ***
C
...
C
 10   KA = 0
      ...
      V(NREDUC) = HALF*DDOT(K, QTR,1,QTR,1)
C
C  ***  SET UP TO TRY INITIAL GAUSS-NEWTON STEP  ***
C
 20   V(DST0) = NEGONE
      IF (IERR .NE. 0) GO TO 90
      T = DLSVMN(P, R, STEP, W(RES))
      IF (T .GE. ONE) GO TO 30
         IF (DNRM2(P, QTR,1) .GE. BIG*T) GO TO 90
C
...
C     ***  TREAT IT AS SUCH WHEN USING DLITVM AND DLIVMU.
 30   CALL DLITVM(P, W, R, QTR)
...
C
 ...
 70      CONTINUE
      CALL DLIVMU(P, STEP, R, STEP)
      T = ONE / DNRM2(P, STEP,1)
      W(PHIPIN) = (T/DST)*T
      LK = PHI*W(PHIPIN)
C
C  ***  COMPUTE U0  ***
...
C
 110  KA = KA + 1
      CALL DCOPY(PP1O2, R,1,W(RMAT),1)
      CALL DCOPY(P, QTR,1,W(RES),1)
C
...
C  ***  COMPUTE STEP  ***
C
 280  CALL DLITVM(P, W(RES), W(RMAT), W(RES))
...
C
      IF (PHI .GT. ZERO) GO TO 310
         IF (KA .GE. KALIM) GO TO 430
              TWOPSI = ALPHAK*DST*DST - DDOT(P, STEP,1,G,1)
...
C
 ...
 320  DO 330 I = 1, P
         ...
         STEP(I) = D(J1) * (W(K)/DST)
 330     CONTINUE
      CALL DLIVMU(P, STEP, W(RMAT), STEP)
      DO 340 I = 1, P
...
C
 430  V(STPPAR) = ALPHAK
 440  V(GTSTEP) = DMIN1(DDOT(P, STEP,1,G,1),ZERO)
