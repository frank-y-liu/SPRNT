      SUBROUTINE DGLRIT(D, G, IV, LIV, LV, P, PS, V, X, Y)
C
...
C  ***  DISCUSSION  ***
C
C        DGLRIT PERFORMS NL2SOL-LIKE ITERATIONS FOR A VARIETY OF
...
C     COMPACTLY BY ROWS), AND DGLRIT BUILDS AN APPROXIMATION, S, TO THE
C     SECOND-ORDER TERM.  THE CALLER ALSO PROVIDES THE FUNCTION VALUE,
C     GRADIENT, AND PART OF THE YIELD VECTOR USED IN UPDATING S.  DGLRIT
...
C
C        FOR UPDATING S, DGLRIT ASSUMES THAT THE GRADIENT HAS THE FORM
...
C     RHO(I,X)*GRAD(R(I,X0)), WHEN CALLING DGLRIT WITH IV(1) = 2 AND
...
C     NL2SNO), ARE NOT REFERENCED BY DGLRIT OR THE SUBROUTINES IT CALLS.
C
C        WHEN DGLRIT IS FIRST CALLED, I.E., WHEN DGLRIT IS CALLED WITH
C     IV(1) = 0 OR 12, V(F), G, AND HC NEED NOT BE INITIALIZED.  TO
C     OBTAIN THESE STARTING VALUES, DGLRIT RETURNS FIRST WITH IV(1) = 1,
...
C     IN COMPUTING A COVARIANCE MATRIX.  IN THIS CASE DGLRIT WILL MAKE A
...
C             FUNCTION VALUE AT X, AND CALL DGLRIT AGAIN, HAVING CHANGED
...
C             CAUSE DGLRIT TO IGNORE V(F) AND TRY A SMALLER STEP.  NOTE
...
C             PUTING G, HC, AND Y THE NEXT TIME DGLRIT RETURNS WITH
...
C             THE CALLER SHOULD THEN CALL DGLRIT AGAIN (WITH IV(1) = 2).
...
C             IV(NFGCAL) TO 0, IN WHICH CASE DGLRIT WILL RETURN WITH
C             IV(1) = 15.
C
C  ***  GENERAL  ***
C
...
C
      EXTERNAL DASSST, DFDHES, DGQTST, DITSUM, DLMSTP, DLSQAR,
     1         DLTVMU, DLVMUL, DPARCK, DSLUPD, DSLVMU, DSTOPX, DVAXPY,
     2          DVSCPY
      LOGICAL DSTOPX
      DOUBLE PRECISION DDOT, DNRM2
C
C DASSST.... ASSESSES CANDIDATE STEP.
C DFDHES.... COMPUTE FINITE-DIFFERENCE HESSIAN (FOR COVARIANCE).
C DGQTST.... COMPUTES GOLDFELD-QUANDT-TROTTER STEP (AUGMENTED MODEL).
C DITSUM.... PRINTS ITERATION SUMMARY AND INFO ON INITIAL AND FINAL X.
C DLMSTP... COMPUTES LEVENBERG-MARQUARDT STEP (GAUSS-NEWTON MODEL).
C DLSQAR... COMPUTES L * L**T FROM LOWER TRIANGULAR MATRIX L.
C DLTVMU... COMPUTES L**T * V, V = VECTOR, L = LOWER TRIANGULAR MATRIX.
C DLVMUL.... COMPUTES L * V, V = VECTOR, L = LOWER TRIANGULAR MATRIX.
C DPARCK.... CHECK VALIDITY OF IV AND V INPUT COMPONENTS.
C DSLUPD... PERFORMS QUASI-NEWTON UPDATE ON COMPACTLY STORED LOWER TRI-
...
C
C/6
      DATA CNVCOD/55/, COVMAT/26/, COVPRT/14/, COVREQ/15/, DIG/37/,
...
C/7
C     PARAMETER (CNVCOD=55, COVMAT=26, COVPRT=14, COVREQ=15, DIG=37,
...
C/6
     ...
     4     WSCALE/56/
C/7
C     PARAMETER (COSMIN=47, DGNORM=1, DSTNRM=2, F=10, FDIF=11, FUZZ=45,
...
C
      ...
      S1 = IV(S)
      IF (IV(INITS) .EQ. 0) CALL DVSCPY(PP1O2, V(S1), ZERO)
      IV(1) = 1
      J = IV(IPIVOT)
      IF (J .LE. 0) GO TO 999
      DO 30 I = 1, P
...
C
 80   CALL DITSUM(D, G, IV, LIV, LV, P, V, X)
 90   K = IV(NITER)
...
C
 ...
 110     CONTINUE
      STEP1 = IV(STEP)
      V(RADIUS) = V(RADFAC) * DNRM2(P, V(STEP1),1)
C
...
C     ***  COPY X TO X0  ***
C
      CALL DCOPY(P, X,1,V(X01),1)
C
C  ***  CHECK DSTOPX AND FUNCTION EVALUATION LIMIT  ***
C
 130  IF (.NOT. DSTOPX(DUMMY)) GO TO 150
         IV(1) = 11
         GO TO 160
C
...
C
         ...
         IPIV1 = IV(IPIVOT)
         CALL DLMSTP(D, G, IV(IERR), IV(IPIV1), IV(KALM), P, V(QTR1),
...
C        *** MARK INVALID THE INFORMATION DGQTST MAY HAVE STORED IN V...
         IV(KAGQT) = -1
         GO TO 250
C
...
C
 240  DIG1 = IV(DIG)
      LMAT1 = IV(LMAT)
      CALL DGQTST(D, V(DIG1), V(H1), IV(KAGQT), V(LMAT1), P, V(STEP1),
     1            V, V(W1))
...
C
 250  IF (IV(IRC) .EQ. 6) GO TO 260
      X01 = IV(X0)
      STEP1 = IV(STEP)
      CALL DVAXPY(P, X, ONE, V(STEP1), V(X01))
...
C
 260  STEP1 = IV(STEP)
      LSTGST = IV(STLSTG)
      X01 = IV(X0)
      CALL DASSST(D, IV, P, V(STEP1), V(LSTGST), V, X, V(X01))
C
...
C
      E = V(PREDUC) - V(FDIF)
      S1 = IV(S)
      CALL DSLVMU(P, Y, V(S1), V(STEP1))
      STTSST = HALF * DDOT(P, V(STEP1),1,Y,1)
      IF (IV(MODEL) .EQ. 1) STTSST = -STTSST
      IF (DABS(E + STTSST) * V(FUZZ) .GE. DABS(E)) GO TO 280
C
C     ***  SWITCH MODELS  ***
C
         IV(MODEL) = 3 - IV(MODEL)
...
C
         ...
         IF (HC1 .LE. 0) GO TO 330
              CALL DSLVMU(P, V(TEMP1), V(HC1), V(STEP1))
              GO TO 340
 330     RMAT1 = IV(RMAT)
         CALL DLTVMU(P, V(TEMP1), V(RMAT1), V(STEP1))
         CALL DLVMUL(P, V(TEMP1), V(RMAT1), V(TEMP1))
C
 340     IF (STPMOD .EQ. 1) GO TO 350
              S1 = IV(S)
              CALL DSLVMU(P, V(TEMP2), V(S1), V(STEP1))
              CALL DVAXPY(P, V(TEMP1), ONE, V(TEMP2), V(TEMP1))
C
C  ***  SAVE OLD GRADIENT AND COMPUTE NEW ONE  ***
C
 350  IV(NGCALL) = IV(NGCALL) + 1
      G01 = IV(W)
      CALL DCOPY(P, G,1,V(G01),1)
      IV(1) = 2
      GO TO 999
C
...
C
 360  G01 = IV(W)
      CALL DVAXPY(P, V(G01), NEGONE, V(G01), G)
      STEP1 = IV(STEP)
      TEMP1 = IV(STLSTG)
      TEMP2 = IV(X0)
...
C        ***  DO GRADIENT TESTS  ***
C
         IF (DNRM2(P, V(TEMP1),1) .LE. V(DGNORM) * V(TUNER4))  GO TO 380
              IF (DDOT(P, G,1,V(STEP1),1)
     1                  .GE. V(GTSTEP) * V(TUNER5))  GO TO 390
 380               V(RADFAC) = V(INCFAC)
C
C  ***  COMPUTE Y VECTOR NEEDED FOR UPDATING S  ***
C
 390  CALL DVAXPY(P, Y, NEGONE, Y, G)
C
C  ***  DETERMINE SIZING FACTOR V(SIZE)  ***
C
C     ***  SET TEMP1 = S * STEP  ***
      S1 = IV(S)
      CALL DSLVMU(P, V(TEMP1), V(S1), V(STEP1))
C
      T1 = DABS(DDOT(P, V(STEP1),1,V(TEMP1),1))
      T = DABS(DDOT(P, V(STEP1),1,Y,1))
      V(SIZE) = ONE
...
C
      ...
      IF (HC1 .LE. 0) GO TO 400
         CALL DSLVMU(P, V(G01), V(HC1), V(STEP1))
         GO TO 410
C
 400  RMAT1 = IV(RMAT)
      CALL DLTVMU(P, V(G01), V(RMAT1), V(STEP1))
      CALL DLVMUL(P, V(G01), V(RMAT1), V(G01))
C
 410  CALL DVAXPY(P, V(G01), ONE, Y, V(G01))
C
C  ***  UPDATE S  ***
C
      CALL DSLUPD(V(S1), V(COSMIN), PS, V(SIZE), V(STEP1), V(TEMP1),
...
C
 440  CALL DFDHES(D, G, I, IV, LIV, LV, P, V, X)
      GO TO (450, 460, 490), I
...
C
 470  H1 = IABS(IV(H))
      ...
      IF (HC1 .LE. 0) GO TO 480
           CALL DCOPY(P*(P+1)/2, V(HC1),1,V(H1),1)
           GO TO 490
 480  RMAT1 = IV(RMAT)
      CALL DLSQAR(P, V(H1), V(RMAT1))
C
