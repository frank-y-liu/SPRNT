...
*********************************************************************
  ...
  This is a special version of top lever SPRNT driver. It will *not* be automatically 
  built when "make" is invoked. If you want to build it, you have to know how to 
  modify the Makefile. If you don't know how, then probably you don't want to touch 
  (and use) this driver anyway.

  Purpose:
    This special SPRNT driver will run the given netlist, but will always load 
    the initial conditions from a "state" file. After completing the unsteady solve,
    it will also save the current state to the same file, overwritten its content. 
    ...
    The use scenario of this SPRNT drive is the following:
        ...
        - run SPRNT using netlist3, load the initial conditios from state file, etc
 
    It's clear that netlist1 and netlist2 should be built for the same river network,
    otherwise all hell will break lose. Therefore, netlist1, netlist2, netlist3,...
    should be identical, except for the unsteady portion of the forcing terms, 
    the boundary conditions and the lateral flows are different.
