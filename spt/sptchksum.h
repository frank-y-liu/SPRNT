...
/****************************************
 a simple class to check multiple char strings
   Typical usage:
      ...
      cs.reset();
      repeat
   can be copied or assigned, in that case the internal checksum will be inherited
   change the sequence of adding checksum will not change the check sum
   hence
   ...
   read large array of strings and calculate the checksum
     ...
     printf("checksum = %x, seed = %x, hex = %s\n", cs.CheckSum(), cs.Seed(), cs.HexCode() );
...
class ChkSum {
 protected:
  ...
  unsigned int      _seed;
  char     _buf[9];
  void     _gen_seed() { srand((unsigned)time(0)); _seed = (unsigned int)rand() ;}

 public:
   ChkSum():_counter(0) { _seed=mycheck_init; _chk=_seed;}
  ChkSum(const ChkSum& other):_counter(0),_chk(other._chk),_seed(other._seed){}
  ChkSum operator=(const ChkSum& rhs) {_counter=0; _chk=rhs._chk; _seed=rhs._seed; return *this;}
  ~ChkSum() {}
  ...
  void reset() { _counter=0; _seed = mycheck_init; _chk=_seed;}
  void reset(unsigned int s) { _counter=0; _seed = s; _chk=_seed;}

  void set_seed(int s) { _seed = s; }
  void rnd_seed(void) { _gen_seed(); _chk=_seed; }

  void add_string(char *str) {
    for (int i=0; str[i]!='\0';i++) _chk += ((int)str[i] * (i+1));
    _counter++;
  }
