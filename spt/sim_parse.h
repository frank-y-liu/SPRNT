...
typedef enum { trapezoidal = 1, rectangular, xy, timeseries } SecondDef;

typedef enum { ft_ascii, ft_integer, ft_real, ft_block } FType;

typedef enum { no = 0, yes, secondary } Required;
...
/*
  ...
  default size = 512
  This template is intended to be used to temporary store data, therefore it is
  not bullet proof. Avoid letting non-friends to use it

  To assigned the data: use ithVal() method
  getIthArray() is provided to quickly access the data, but it is not safe

  Reset() will reset the template for next use. Memory is not freed till the
  instance is deleted

 */
template <class T, unsigned int NSEQ = 2> class simpleTuple {
protected:
  T **_data;
  T _scale;
...
public:
  ...
  // methods:
  T &ithVal(int jj, int kk) {
    assert((unsigned int)jj < NSEQ);
    _grow(kk);
    return (_data[jj][kk]);
  }
  T *getIthArray(int jj) {
    assert((unsigned int)jj < NSEQ);
    return (_data[jj]);
  } /* be careful */
...
public:
  ...
  ~NameStore() {
    if (_store) {
      for (int jj = 0; jj < _allocated_size; jj++)
        if (_store[jj])
          free(_store[jj]);
      free(_store);
    }
  ...
  void Init(int sz, int word_length = MAX_WORD_LENGTH) {
    ...
    for (int jj = 0; jj < _allocated_size; jj++)
      _store[jj] = (char *)malloc(sizeof(char) * word_length);
  }
...
public:
  ...
  */
  ...
  inline int Cmp_Head(
      const char *t2) { // compare to see if the header matches the given char
    int rc = 1;
    char *t1 = _header;
    while (1) {
      if (*t2 == 0 || rc == 0)
        break;
      rc *= (toupper(*t1++) == *t2++ ? 1
                                     : 0); /* logic AND, need all characters */
    }
    return (rc == 1);
  }
